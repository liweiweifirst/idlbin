...
;dir_name = '/Users/jkrick/iwic/iwicDA/IRAC019000'
;aor_list = 'aor_t37r35b60_50', 'aor_t37r35b60_50_bright','aor_t37r35b60_50_cal'
...
;make smoothed images to work with.
         ...
         ;need result of gaussian fit here so I can't do this in the first loop
                  if finite((*rawdataptr[i])[x,y]) gt 0 then begin
                     ...
                     if  (*rawdataptr[i])[x,y] gt 10*result(0) + medianarr(i) then begin
                                ;using 3.5 sigma is good for hot
                                ;pixels, but we only care here about
                                ;truly hot with no leftover dynamic
                                ;range for a real object.
                        hotpixel = hotpixel + 1
                     endif
                  endif
     ...
     ;if that same pixel is hot on all of the images where finite then it must be
                                ;a hot pixel
;               if pos lt 3 then nfin = 5
;               if pos eq 3 then nfin = 16
               nfin = 5  
