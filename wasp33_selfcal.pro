pro wasp33_selfcal, binning=binning, sine=sine
...
;-------------------------------------------------------------------------------------
  xarr = AORwasp33.xcen
  yarr = AORwasp33.ycen
  timearr = AORwasp33.timearr
  fluxerr = AORwasp33.fluxerr
  flux = AORwasp33.flux
  corrflux = AORwasp33.corrflux
  corrfluxerr = AORwasp33.corrfluxerr
  bmjd = AORwasp33.bmjdarr
  utcs = AORwasp33.utcsarr

;get rid of outliers
 print, ' fluxarr', n_elements(flux)
 
 ; the position outliers
;  good = where(AORwasp33[a].xcen lt mean(AORwasp33[a].xcen)
;  +3.0*stddev(AORwasp33[a].xcen) and AORwasp33[a].xcen gt
;  mean(AORwasp33[a].xcen) -3.0*stddev(AORwasp33[a].xcen) and
;  AORwasp33[a].ycen lt mean(AORwasp33[a].ycen)
;  +3.0*stddev(AORwasp33[a].ycen) and AORwasp33[a].ycen gt
;  mean(AORwasp33[a].ycen) - 3.0*stddev(AORwasp33[a].ycen),ngood_pmap,
;                            complement=bad) 
...
;print, 'flux', flux(q)
 ...
 print, 'positions', mean(xarr) + 1.0*stddev(xarr),  mean(xarr) -1.0*stddev(xarr),  mean(yarr) +1.0*stddev(yarr), mean(yarr) - 1.0*stddev(yarr)
  ...
  good = where(xarr lt 15.3 and xarr gt mean(xarr) -1.0*stddev(xarr) and yarr lt mean(yarr) +1.0*stddev(yarr) and yarr gt mean(yarr) - 1.0*stddev(yarr),ngood_pmap, complement=bad) 
  print, 'bad position',n_elements(bad), n_elements(good)

;  xarr = AORwasp33[a].xcen[good]
;  yarr = AORwasp33[a].ycen[good]
;  timearr = AORwasp33[a].timearr[good]
;  fluxerr = AORwasp33[a].fluxerr[good]
;  flux = AORwasp33[a].flux[good]
;  corrflux = AORwasp33[a].corrflux[good]
;  corrfluxerr = AORwasp33[a].corrfluxerr[good]
;  bmjd = AORwasp33[a].bmjdarr[good]
;  utcs = AORwasp33[a].utcsarr[good]
...
;-------------------------------------------------------------------------------------
  ...
  stoptransit = min(where(timearr gt 6.5)) ;6.5
  ...
  starttransit = max(where(timearr lt 17.5)) ;3.0
  stoptransit = min(where(timearr gt 21.0)) ;6.5
  xarr = [xarr[0:starttransit],xarr[stoptransit:n_elements(timearr)-1]]
  yarr = [yarr[0:starttransit],yarr[stoptransit:n_elements(timearr)-1]]
  fluxerr = [fluxerr[0:starttransit],fluxerr[stoptransit:n_elements(timearr)-1]]
  flux = [flux[0:starttransit],flux[stoptransit:n_elements(timearr)-1]]
 ...
 starttransit = max(where(timearr lt 32.)) ;3.0
  stoptransit = min(where(timearr gt 36.0)) ;6.5
  xarr = [xarr[0:starttransit],xarr[stoptransit:n_elements(timearr)-1]]
  yarr = [yarr[0:starttransit],yarr[stoptransit:n_elements(timearr)-1]]
  fluxerr = [fluxerr[0:starttransit],fluxerr[stoptransit:n_elements(timearr)-1]]
  flux = [flux[0:starttransit],flux[stoptransit:n_elements(timearr)-1]]
...
;really the jumps in pcrspeakup observing to move back to sweet spot
 starttransit = max(where(timearr lt 12.3)) ;3.0
  stoptransit = min(where(timearr gt 12.4)) ;6.5
  xarr = [xarr[0:starttransit],xarr[stoptransit:n_elements(timearr)-1]]
  yarr = [yarr[0:starttransit],yarr[stoptransit:n_elements(timearr)-1]]
  fluxerr = [fluxerr[0:starttransit],fluxerr[stoptransit:n_elements(timearr)-1]]
  flux = [flux[0:starttransit],flux[stoptransit:n_elements(timearr)-1]]
 ...
 starttransit = max(where(timearr lt 24.7)) ;3.0
  stoptransit = min(where(timearr gt 25.)) ;6.5
  xarr = [xarr[0:starttransit],xarr[stoptransit:n_elements(timearr)-1]]
  yarr = [yarr[0:starttransit],yarr[stoptransit:n_elements(timearr)-1]]
  fluxerr = [fluxerr[0:starttransit],fluxerr[stoptransit:n_elements(timearr)-1]]
  flux = [flux[0:starttransit],flux[stoptransit:n_elements(timearr)-1]]
...
;     st2 = plot(bin_time, bin_model_y+14.5,'1sb',/overplot, sym_size = 0.2, sym_filled = 1.)
  ...
  ;x vs y
   ...
   ;-----------------
     ...
     p1 = plot(x, y,  '1s',  xtitle = 'Time(hrs)', ytitle = ' Flux',  sym_size = 0.2, sym_filled = 1.,title = titlename, yrange = [0.172,0.178]) ;, xrange = [2,4]
     f1 = plot(x,bin_model,  '1sb',/overplot , sym_size = 0.2, sym_filled = 1.) ;-0.0027
     f2 = plot(x, bin_sub+0.174, '1sr',/overplot , sym_size = 0.2, sym_filled = 1.)
  ...
  save, selfcal_timearr, filename='/Users/jkrick/irac_warm/pcrs_planets/wasp33/selfcal_timearr.sav'
  save, selfcal_flux, filename='/Users/jkrick/irac_warm/pcrs_planets/wasp33/selfcal_flux.sav'
  save, bin_time, filename='/Users/jkrick/irac_warm/pcrs_planets/wasp33/bin_time.sav'
  save, bin_sub, filename='/Users/jkrick/irac_warm/pcrs_planets/wasp33/bin_sub.sav'
