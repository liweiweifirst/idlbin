pro plot_exoplanet_simple, planetname, bin_level, apradius, chname
...
;now try plotting
 ...
 ;------------------------------------------------------
     ...
     corrnormoffset = .008
     setynormfluxrange = [0.997, 1.002] ;[0.97, 1.005]
     ...
     if a eq startaor then begin ; for the first AOR
        ...
        if pmapcorr eq 1 then begin
           ...
           pr = plot((bin_timearr - time_0)/60./60., (bin_corrfluxp/plot_corrnorm) ,  '1s', $
                     ...
                     ytitle = 'Normalized Corrected Flux', title = aorname(a), $
