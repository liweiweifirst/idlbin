...
dec_ref = 50.12874
  ...
  nfits = 870L*63L    
  print, 'nfits',nfits
  AORhat22 = replicate({hat22ob, ra:0D,dec:0D,xcen:dblarr(nfits),ycen:dblarr(nfits),flux:dblarr(nfits),fluxerr:dblarr(nfits), corrflux:dblarr(nfits), corrfluxerr:dblarr(nfits), sclktime_0:0D, timearr:dblarr(nfits),aor:' ', bmjdarr:dblarr(nfits), utcsarr:dblarr(nfits), bkgd:dblarr(nfits), bkgderr:dblarr(nfits)},n_elements(aorname))


  for a = 0, n_elements(aorname) - 1 do begin
     print, 'working on ',aorname(a)
     dir = '/Users/jkrick/irac_warm/pcrs_planets/hat22/'+ string(aorname(a) ) 
     CD, dir                    ; change directories to the correct AOR directory
     command  =  "find ch2/bcd -name '*_bcd.fits' > /Users/jkrick/irac_warm/pcrs_planets/hat22/bcdlist.txt"
     spawn, command
     command2 =  "find  ch2/bcd -name '*bunc.fits' > /Users/jkrick/irac_warm/pcrs_planets/hat22/bunclist.txt"
     spawn, command2

     readcol,'/Users/jkrick/irac_warm/pcrs_planets/hat22/bcdlist.txt',fitsname, format = 'A', /silent
     readcol,'/Users/jkrick/irac_warm/pcrs_planets/hat22/bunclist.txt',buncname, format = 'A', /silent
...
;--------------------------------
 ...
 ;--------------------------------
    ...
    if n_elements(xarr) lt nfits then begin
       ...
       AORhat22[a] ={hat22ob,  ra_ref,dec_ref,xarr2,yarr2,fluxarr2,fluxerrarr2, corrfluxarr2, corrfluxerrarr2, sclk_0, timearr2,aorname(a), bmjd2,utcs2,back2, backerr2}
    endif else begin
       AORhat22[a] ={hat22ob,  ra_ref,dec_ref,xarr,yarr,fluxarr,fluxerrarr, corrfluxarr, corrfluxerrarr, sclk_0, timearr,aorname(a),bmjd, utcs,backarr, backerrarr}
    endelse

  endfor                        ;for each AOR
  
  ;test
  print, AORhat22[0].ra, AORhat22[0].xcen[26]
  
  save, AORhat22, filename='/Users/jkrick/irac_warm/pcrs_planets/hat22/hat22_phot_ch2.sav'
  undefine, AORhat22
