...
;  fits_read, '/Users/jkrick/idlbin/pmap_fits/pmap_ch2_0p1s_x4_500x500_0043_120124.fits', pmapdata, pmapheader
  ...
  restore, '/Users/jkrick/irac_warm/pcrs_planets/hat22/hat22_phot_ch2.sav'
  aorname = ['r45675008', 'r45674752'] ;ch2
...
;set phase = 0.0 in the middle of the transit
  for a = 0, n_elements(aorname) - 1 do  begin
      AORhat22[a].bmjdarr = ((AORhat22[a].bmjdarr - utmjd_center) / period) 
   endfor

print, 'phase', AORhat22[0].bmjdarr[0:100]


;  fits_read, '/Users/jkrick/idlbin/pmap_fits/pmap_ch2_0p1s_x4_500x500_0043_120124.fits', pmapdata, pmapheader
;  c = contour(pmapdata, /fill, n_levels = 21, rgb_table = 0, xtitle = 'X (pixel)', ytitle = 'Y (pixel)', aspect_ratio = 1, xrange = [0,500], yrange = [0,500])
;  for a = 0, n_elements(aorname) - 1 do begin
;     xcen500 = 500.* (AORhat22[a].xcen - 14.5)
;     ycen500 = 500.* (AORhat22[a].ycen - 14.5)
...
;        am = plot( (AORhat22[a].timearr - AORhat22[0].timearr(0))/60./60., AORhat22[a].xcen,'6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a], xtitle = 'Time(hrs)', ytitle = 'X pix', yrange = [14.8, 15.3], xrange = [0,10])
;     endif else begin
;        am = plot( (AORhat22[a].timearr - AORhat22[0].timearr(0))/60./60., AORhat22[a].xcen,'6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a],/overplot)
;     endelse
...
;  for a = 0, n_elements(aorname) - 1 do begin
;     if a eq 0 then begin
;        am = plot( (AORhat22[a].timearr - AORhat22[0].timearr(0))/60./60., AORhat22[a].ycen,'6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a], xtitle = 'Time(hrs)', ytitle = 'Y pix',  yrange = [14.8, 15.3], xrange = [0,10])
;     endif else begin
;        am = plot( (AORhat22[a].timearr - AORhat22[0].timearr(0))/60./60., AORhat22[a].ycen,'6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a],/overplot)
;     endelse
...
;  for a = 0, n_elements(aorname) - 1 do begin
;     if a eq 0 then begin
;        am = plot( (AORhat22[a].timearr - AORhat22[0].timearr(0))/60./60., AORhat22[a].bkgd,'6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a], xtitle = 'Time(hrs)', ytitle = 'Background', xrange = [0,10])
;     endif else begin
;        am = plot( (AORhat22[a].timearr - AORhat22[0].timearr(0))/60./60., AORhat22[a].bkgd,'6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a],/overplot)
;     endelse
...
;  for a = 0, n_elements(aorname) - 1 do begin
;     if a eq 0 then begin
;        am = errorplot( (AORhat22[a].timearr - AORhat22[0].timearr(0))/60./60., AORhat22[a].flux,AORhat22[a].fluxerr, '6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a], xtitle = 'Time(hrs)', ytitle = 'Flux', xrange = [0,10], yrange = [0.03,0.045], errorbar_capsize = 0.)
;        am = errorplot( (AORhat22[a].timearr - AORhat22[0].timearr(0))/60./60., AORhat22[a].corrflux-0.003,AORhat22[a].corrfluxerr,'6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a], /overplot, errorbar_capsize = 0.)
;     endif else begin
;        am = errorplot( (AORhat22[a].timearr - AORhat22[0].timearr(0))/60./60., AORhat22[a].flux,AORhat22[a].fluxerr,'6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a],/overplot, errorbar_capsize = 0.)
; am = errorplot( (AORhat22[a].timearr - AORhat22[0].timearr(0))/60./60., AORhat22[a].corrflux-0.003,AORhat22[a].corrfluxerr,'6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a],/overplot, errorbar_capsize = 0.)
;     endelse
...
;------
;a = where(finite(AORhat22[1].corrflux) gt 0 and finite(AORhat22[1].flux) gt 0)

; print, (abs(AORhat22[1].corrflux[a[0:100]] - AORhat22[1].flux[a[0:100]])/ AORhat22[1].flux[a[0:100]])*100
;plothist,(abs(AORhat22[1].corrflux[a[0:100]] - AORhat22[1].flux[a[0:100]])/ AORhat22[1].flux[a[0:100]])*100, xhist, yhist, bin = 0.1,/fill


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;try this diffferently, try putting the AORs togetether as one array; then bin

;timearr = [AORhat22[0].timearr, AORhat22[1].timearr]
;fluxarr = [AORhat22[0].flux, AORhat22[1].flux]
;fluxerrarr = [AORhat22[0].fluxerr, AORhat22[1].fluxerr]
;corrfluxarr = [AORhat22[0].corrflux, AORhat22[1].corrflux]
;corrfluxerrarr = [AORhat22[0].corrfluxerr, AORhat22[1].corrfluxerr]
;xarr = [AORhat22[0].xcen, AORhat22[1].xcen]
;yarr = [AORhat22[0].ycen, AORhat22[1].ycen]
;bkgd = [AORhat22[0].bkgd, AORhat22[1].bkgd]
;bmjd = [AORhat22[0].bmjdarr, AORhat22[1].bmjdarr]

;actually don't include the initial 30min AOR.
timearr = [AORhat22[1].timearr]
fluxarr = [ AORhat22[1].flux]
fluxerrarr = [ AORhat22[1].fluxerr]
corrfluxarr = [ AORhat22[1].corrflux]
corrfluxerrarr = [AORhat22[1].corrfluxerr]
xarr = [ AORhat22[1].xcen]
yarr = [AORhat22[1].ycen]
bkgd = [ AORhat22[1].bkgd]
bmjd = [ AORhat22[1].bmjdarr]

print, 'n', n_elements(timearr), n_elements(AORhat22[0].timearr), n_elements(AORhat22[1].timearr)
;now for the binning
...
print, 'c', c, n_elements(bin_corrflux)
...
;     pl = errorplot((bin_timearr - bin_timearr(0))/60./60., bin_flux/median(bin_flux),bin_fluxerr, '6r1s', sym_size = 0.1,   sym_filled = 1, xrange = [0,9],  yrange = [0.99, 1.02], xtitle = 'Time (hrs)', ytitle = 'Normalized Flux', errorbar_capsize = 0.)
      ...
      pl = plot((bin_timearr - bin_timearr(0))/60./60., bin_flux/median(bin_flux), '6r1s', sym_size = 0.1,   sym_filled = 1,  xrange = [0,7],yrange = [0.98, 1.01], xtitle = 'Time (hrs)', ytitle = 'Normalized Flux', title = 'Hat 22')
