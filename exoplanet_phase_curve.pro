...
PRO exoplanet_transit_eclipse,phase,ar_semimaj,r,rp_rstar,inclination,star_frac,planet_frac,itransit,ieclipse
  ;;  INPUTS:
  ;;          PHASE      = Array of orbital phase values, modulo 2*!dpi (radians)
  ...
  ;;
  ;;   OUTPUTS:
  ;;          STAR_FRAC   = Flux of star as a function of PHASE, relative to unobstructed stellar flux
  ;;          PLANET_FRAC = Flux of planet as a function of PHASE, relative to unobstructed planetary flux
...
;;; Recondition phases to go between 0 and 2pi
  ...
  degrad = !dpi/180d0
  nt = N_ELEMENTS(phase)
  ;;; Projected distance between centers of planet and star (in units of Rstar):  r/rstar * SQRT(sin^2(phase) + cos^2(i)*cos^2(phase)) 
  z = r * ar_semimaj * SQRT(1 - COS(phase)^2 * SIN(inclination * degrad)^2)
  ...
  i1 = WHERE( (z GT (1+P)) OR (0.5*!dpi LT phase AND 1.5*!dpi GE phase),n1)     ;; Out of transit
  i2 = WHERE( z GT (1-P) AND z LE (1+P) AND (phase GT 1.5*!dpi OR phase LE 0.5*!dpi),n2)   ;;; Partial transit
  i3 = WHERE( z LE (1-P)  AND (phase GT 1.5*!dpi OR phase LE 0.5*!dpi),n3)  ;; Full Transit
  ...
  i1 = WHERE( (z GT (1+P)) OR (phase GT 1.5*!dpi OR phase LE 0.5*!dpi),n1)   ;; Out of contact
  i2 = WHERE( z GT (1-P) AND z LE (1+P) AND (0.5*!dpi LT phase AND 1.5*!dpi GE phase),n2)   ;;; Partial occultation
  i3 = WHERE( z LE (1-P)  AND (0.5*!dpi LT phase AND 1.5*!dpi GE phase),n3)    ;;; full occultaiton
