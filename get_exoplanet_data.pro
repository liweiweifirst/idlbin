PRO get_exoplanet_data,EXOSYSTEM=exosystem,MSINI=msini,MSTAR=mstar,TRANSIT_DEPTH=transit_depth,RP_RSTAR=rp_rstar,AR_SEMIMAJ=ar_semimaj,$
...
ENDIF ELSE BEGIN
      IF KEYWORD_SET(transit_DEPTH) THEN RP_RSTAR=SQRT(transit_depth)
      b_impact = ar_semimaj * cos(inclination/!radeg)
      IF b_impact GT 1+rp_rstar THEN known_transit = 0
      IF ~known_transit then t14 = 0 ELSE t14 = P_ORBIT/!dpi * ACOS(SQRT(1 - (1+rp_rstar)^2/ar_semimaj^2)/SIN(inclination/!radeg))
ENDELSE
