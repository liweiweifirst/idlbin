 ...
 planetinfo = create_planetinfo()
  ...
  for s = 0, n_elements(savefilename) -1 do begin
     ...
     if s eq 0 then begin
        h = plot(xhist, yhist, thick = 2, xtitle = 'Aperture flux', ytitle = 'Number', title = 'WASP-14b', $
           color = 'Navy', name = string('campaign dark corrflux '+string(result(1))), xrange = [0.98,1.02],/ylog);/xlog)
        t= text(1.007, 5500, 'cmpgn corr' + string(result(1)), /data, color = 'Navy')
     endif else begin

        h2 = plot(xhist, yhist, color = 'red',thick = 2,/overplot, name = string('superdark corrflux' + string(result(1))))
        t= text(1.007, 5000, 'sperdrk corr' + string(result(1)), /data, color = 'red')

     endelse

     plothist, fluxarr, xhist, yhist, bin = 0.0001, /noplot
     start = [0.058, 0.0003, 6000]
     error = fltarr(n_elements(xhist)) + 1.0             ; uniform errors
     result= MPFITFUN('mygauss',xhist,yhist, error, start,/quiet)
     print, 'sigma', result(1)
     ...
     if s eq 0 then begin
        h3 = plot(xhist, yhist, color = 'blue',thick = 2,/overplot, name = string('campaign flux' + string(result(1))))
        t= text(1.007, 4500, 'cmpgn flux' + string(result(1)), /data, color = 'blue')

     endif else begin
        h4 = plot(xhist, yhist, color = 'orange',thick = 2,/overplot, name = string('superdark flux' + string(result(1))))
         t= text(1.007, 4000, 'sperdrk flux' + string(result(1)), /data, color = 'orange')
    endelse
