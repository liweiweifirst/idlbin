...
; CALLING SEQUENCE:
;       multiplot_jim,[pmulti][,/help][,/initialize][,/reset][,/rowmajor][,gutter=gutter]
;                    [,/narrowmargin] [,xsqueeze=xsqueeze] [,ysqueeze=ysqueeze]
;                    [,xshift=xshift] [,yshift=yshift] [,position=position]
; Examples:
...
;       gutter = size (in normalized coordinates) of the "gutter region"
;                in between panels
;       /nomargin = eliminate standard extra left and bottom margin,
;       position plots symmetrically
;       /narrowmargin = Remove extra space on left margin
;       xsqueeze = multiplicative factor to shrink or expand x size of
;       plot region
;       ysqueeze = multiplicative factor to shrink or expand y size of
;       plot region
;       xshift = offset (normal coordinates) in x direction of region
;       yshift = offset (normal coordinates) in y direction of region
;       position = position of current plot (in !p.position format, normalized coords)
; Outputs:
...
;-
pro multiplot_jim,help=help,pmulti $
                  ,initialize=initialize,reset=reset,default=default,narrowmargin=narrowmargin $
                  ,rowmajor=rowmajor,verbose=verbose,gutter=gutter,position=pos,nomargin=nomargin,$
                  xsqueeze=xsqueeze,ysqueeze=ysqueeze,xshift=xshift,yshift=yshift
;
;       =====>> COMMON
;
common multiplot $
  ...
  ,guttr        ; amount of space in between plots, normalized units
;
;       =====>> HELP
;
;on_error,2
...
if init or keyword_set(initialize) then begin
  ...
  !p.position = 0                               ; reset
  !p.multi = 0
;;;; JIM added !x.margin,!y.margin keywords here:
  IF keyword_set(narrowmargin) EQ 1 THEN BEGIN
     !x.margin = [5,2]
     !y.margin = [3,2]
  ENDIF
;;;; JIM added xsqueeze, ysqueeze, xshift,yshift keywords
  plot,/nodata,xstyle=4,ystyle=4,!x.range,!y.range,$
       /noerase                 ; set window & region
;;; JIM Addition:
  IF N_ELEMENTS(xsqueeze) EQ 0 THEN xsqueeze=1.0
  IF N_ELEMENTS(ysqueeze) EQ 0 THEN ysqueeze=1.0
  IF N_ELEMENTS(xshift) EQ 0 THEN xshift=0.0
  IF N_ELEMENTS(yshift) EQ 0 THEN yshift=0.0
  plot,/nodata,xstyle=4,ystyle=4,!x.range,!y.range,$
       /noerase                 ; set window & region
  dxreg = !x.region[1]-!x.region[0]
  dxwin_orig = !x.window[1]-!x.window[0]
  dxwin_new = dxwin_orig*xsqueeze
  dyreg = !y.region[1]-!y.region[0]
  dywin_orig = !y.window[1]-!y.window[0]
  dywin_new = dywin_orig*ysqueeze
;  print,'p region',!p.region
;  print,'xregion',!x.region
;  !p.region = [!x.region[0]+xshift>0,!y.region[0]+yshift>0,$
;               !x.region[0]+xshift+dxreg*xsqueeze<1,!y.region[0]+yshift+dyreg*ysqueeze<1]
;  plot,/nodata,xstyle=4,ystyle=4,!x.range,!y.range,$
;       /noerase                 ; set window & region
;  print,'p region',!p.region
;  print,'xregion',!x.region
IF KEYWORD_SET(nomargin) THEN BEGIN
   ...
   ym1 = ym2
ENDIF ELSE BEGIN
   xm1 = min(!x.window)-min(!x.region)
   ym1 = min(!y.window)-min(!y.region)
ENDELSE
xm1 += xshift
ym1 += yshift
xm2 = dxreg - (xm1+dxwin_new)   ;max(!x.region)-max(!x.window)
ym2 = dyreg - (ym1+dywin_new)   ;max(!y.region)-max(!y.window)
margins = [xm1,ym1,xm2,ym2]    ; in normalized coordinates
...
;  print,margins,'=margins'
;;; JIM addition
    IF not keyword_set(gutter) THEN guttr = 0 else guttr = gutter
;;;
...
;
onbottom = (row eq rows) || (rows eq 1)
onleft = (col eq 1) || (cols eq 1)

;;;; Jim addition
nguttrx = cols - 1
xguttr = nguttrx*guttr
xplotsz = (cols*idx[0] - xguttr)/cols  ;;; size of each plot window in x direction
nguttry = rows - 1
yguttr = nguttry*guttr
yplotsz = (rows*idx[1] - yguttr)/rows  ;;; size of each plot window in y direction
pos = [(col-1)*(xplotsz+guttr),(rows-row)*(yplotsz+guttr),(col-1)*(xplotsz+guttr)+xplotsz,(rows-row)*(yplotsz+guttr)+yplotsz] $
      + [margins[0],margins[1],margins[0],margins[1]]
      
;ontop = (row eq 1) || (rows eq 1)
;onright = (col eq cols) || (cols eq 1)
;leftneighbor = ~ onleft && (cols gt 1)
;rightneighbor = ~ onright && (cols gt 1)
;aboveneighbor = ~ ontop && (rows gt 1)
;belowneighbor = ~ onbottom && (rows gt 1)
;pos = pos + 0.5*guttr*[leftneighbor,belowneighbor,-1*rightneighbor,-1*aboveneighbor]
;;;;
