function nearest_neighbors_xyfwhm_DT,x,y,xfwhm, yfwhm, chname, DISTANCES=nearest_d,NUMBER=k
  ...
  for point=0L,n-1 do begin 
     ...
     for i=1,k do begin 
        ...
        if cnt gt 0 then begin 
           new=new[wh]
           p=[p[1:*],new]
           d=[d[1:*],(x[new]-x[point])^2+((y[new]-y[point])/b)^2+ (xfwhm[p]-xfwhm[point])^2+ (yfwhm[p]-yfwhm[point])^2]
        endif else begin 
           p=p[1:*]
           d=d[1:*]
        endelse 
