...
;  AORwasp15 = replicate({wasp15ob, ra:0D,dec:0D,xcen:dblarr(nfits),ycen:dblarr(nfits),flux:dblarr(nfits),fluxerr:dblarr(nfits), corrflux:dblarr(nfits), corrfluxerr:dblarr(nfits), sclktime_0:0D, timearr:dblarr(nfits),aor:' ', bmjdarr:dblarr(nfits), utcsarr:dblarr(nfits), bkgd:dblarr(nfits), bkgderr:dblarr(nfits)},n_elements(aorname))

AORwasp15 = replicate({junk,  ra:0D,dec:0D,xcen:ptr_new(dblarr(10)),y:ptr_new(dblarr(10)),flux:ptr_new(dblarr(10)),fluxerr:ptr_new(dblarr(10)), corrflux:ptr_new(dblarr(10)), corrfluxerr:ptr_new(dblarr(10)), sclktime_0:0D, timearr:ptr_new(dblarr(10)),aor:' ', bmjdarr:ptr_new(dblarr(10)), utcsarr:ptr_new(dblarr(10)), bkgd:ptr_new(dblarr(10)), bkgderr:ptr_new(dblarr(10))},n_elements(aorname))


  for a = 0, 0 do begin; n_elements(aorname) - 1 do begin
     print, 'working on ',aorname(a)
     dir = '/Users/jkrick/irac_warm/pcrs_planets/wasp15/'+ string(aorname(a) ) 
     CD, dir                    ; change directories to the correct AOR directory
     command  =  "find ch2/bcd -name '*_bcd.fits' > /Users/jkrick/irac_warm/pcrs_planets/wasp15/bcdlist.txt"
     spawn, command
     command2 =  "find  ch2/bcd -name '*bunc.fits' > /Users/jkrick/irac_warm/pcrs_planets/wasp15/bunclist.txt"
     spawn, command2

     readcol,'/Users/jkrick/irac_warm/pcrs_planets/wasp15/bcdlist.txt',fitsname, format = 'A', /silent
     readcol,'/Users/jkrick/irac_warm/pcrs_planets/wasp15/bunclist.txt',buncname, format = 'A', /silent
...
;--------------------------------
 ...
 ;--------------------------------
    ...
    if n_elements(xarr) lt nfits then begin
       ...
       AORwasp15[a] ={junk,  ra_ref,dec_ref,xarr2,yarr2,fluxarr2,fluxerrarr2, corrfluxarr2, corrfluxerrarr2, sclk_0, timearr2,aorname(a), bmjd2,utcs2,back2, backerr2}
    endif else begin
       AORwasp15[a] ={wasp15ob,  ra_ref,dec_ref,xarr,yarr,fluxarr,fluxerrarr, corrfluxarr, corrfluxerrarr, sclk_0, timearr,aorname(a),bmjd, utcs,backarr, backerrarr}
    endelse

  endfor                        ;for each AOR
  
  ;test
  print, 'AORwasp15[0].xcen',AORwasp15[0].xcen
  
 ; save, AORwasp15, filename='/Users/jkrick/irac_warm/pcrs_planets/wasp15/wasp15_phot_ch2.sav'
 ; undefine, AORwasp15
