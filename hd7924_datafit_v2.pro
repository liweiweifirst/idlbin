...
;the plots!
 ; p1 = errorplot(x, y, yerr, '1-s',  xtitle = 'Time(hours)', ytitle = ' Flux', title = 'Data Fit', errorbar_capsize = .05 , sym_size = 0.4, sym_filled = 1.,yrange = [1.4, 1.5]) ;, xrange = [2,4]
      p1 = plot(x, y,  '1-s',  xtitle = 'Time(hours)', ytitle = ' Flux', title = 'Data Fit',  sym_size = 0.4, sym_filled = 1.,yrange = [1.4, 1.5]) ;, xrange = [2,4]
 ;--------------------------------------------------------------------------------------------------
...
;  pa0 = [median(y), 1.,1.,1.,1.,1.,0.006,20.618,0.5]
  ...
  f2 = plot(x, sub+1.427, '1-sr',/overplot , sym_size = 0.4, sym_filled = 1.)
  
;now for some binning;
  binned_sub = fltarr(nfits)
  binned_timearr = fltarr(nfits)

  for si = 0., nfits - 1 do begin
     print, 'working on si', si,nframes, n_elements(sub), n_elements(timearr)
     idata = sub[si*nframes:si*nframes + (nframes - 1)]
     binned_sub[si] = mean(idata,/nan)
     binned_timearr[si] = mean(snapshots[0].timearr[si*nframes:si*nframes + (nframes - 1)])
              
  endfor                        ;for each fits image

  d = plot(binned_timearr, binned_sub)
  save, binned_sub, filename = '/Users/jkrick/irac_warm/snapshots/hd7924/hd7924_binnedsub.sav'
