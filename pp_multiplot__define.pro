...
;    `pp_multiplot::init`, `pp_multiplot::plot`, `pp_multiplot::getproperty`,
...
case n_params() of
  0:ret=plot(TEST=test, _EXTRA=ex,position=position,/current,$
   xtickfont_size=bottom*12,ytickfont_size=left*12,xrange=(self.xranges)[column],yrange=(self.yranges)[line])
  1:ret=plot(arg1,TEST=test, _EXTRA=ex,position=position,/current,$
   xtickfont_size=bottom*12,ytickfont_size=left*12,xrange=(self.xranges)[column],yrange=(self.yranges)[line])
  2:ret=plot(arg1,arg2,TEST=test, _EXTRA=ex,position=position,/current,$
   xtickfont_size=bottom*12,ytickfont_size=left*12,xrange=(self.xranges)[column],yrange=(self.yranges)[line])
  3:ret=plot(arg1,arg2,arg3,TEST=test, _EXTRA=ex,position=position,/current,$
   xtickfont_size=bottom*12,ytickfont_size=left*12,xrange=(self.xranges)[column],yrange=(self.yranges)[line])
endcase
...
self.setendticks,xendticks,ret,0
self.setendticks,yendticks,ret,1
...
function pp_multiplot::errorplot,arg1, arg2, arg3, TEST=test, _EXTRA=ex,$
 multi_index=mindex,xrange=xrange,yrange=yrange,propagate=propagate,$
 xendticks=xendticks,yendticks=yendticks
compile_opt idl2, logical_predicate
...
self.owindow.select,/clear ;Just to make this window the current one
case n_params() of
  0:ret=errorplot(TEST=test, _EXTRA=ex,position=position,/current,$
   xtickfont_size=bottom*12,ytickfont_size=left*12,xrange=(self.xranges)[column],yrange=(self.yranges)[line])
  1:ret=errorplot(arg1,TEST=test, _EXTRA=ex,position=position,/current,$
   xtickfont_size=bottom*12,ytickfont_size=left*12,xrange=(self.xranges)[column],yrange=(self.yranges)[line])
  2:ret=errorplot(arg1,arg2,TEST=test, _EXTRA=ex,position=position,/current,$
   xtickfont_size=bottom*12,ytickfont_size=left*12,xrange=(self.xranges)[column],yrange=(self.yranges)[line])
  3:ret=errorplot(arg1,arg2,arg3,TEST=test, _EXTRA=ex,position=position,/current,$
   xtickfont_size=bottom*12,ytickfont_size=left*12,xrange=(self.xranges)[column],yrange=(self.yranges)[line])
endcase
...
;    ax: in, required
;      The axis to act upon in the given plot object: 0 for x, 1 for y.
;
;-
pro pp_multiplot::setendticks,endticks,opl,ax
compile_opt idl2,logical_predicate,hidden
if ((endticks ne 3) && isa(((opl.axes)[0]).tickname)) then begin
  tn=((opl.axes)[ax]).tickname
  if ((endticks eq 0)||(endticks eq 2)) then tn[0]=''
  if ((endticks eq 0)||(endticks eq 1)) then tn[-1]=''
  ((opl.axes)[ax]).tickname=tn
endif
end
...
if (n_elements(xranges) ne 0) then foreach el,self.oplots,i do if isa(el) then begin
  ...
  if ~array_equal(el.xrange,xranges[column]) then begin
    ...
    self.setendticks,(self.xendticks)[i],el,0
    self.setendticks,(self.yendticks)[i],el,1
  endif
endif
...
if (n_elements(mindex) eq 1) then begin
  if isa((self.oplots)[mindex]) then begin
    ...
    self.setendticks,(self.xendticks)[mindex],(self.oplots)[mindex],0
    self.setendticks,(self.yendticks)[mindex],(self.oplots)[mindex],1
  endif else begin
