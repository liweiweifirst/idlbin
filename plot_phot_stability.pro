...
;fits_read, '/Users/jkrick/irac_warm/pmap/pmap_fits/occu_ch2_0p1s_x4_500x500_0043_121120.fits', pmapdata, pmapheader
...
;   print, 'bad', n_elements(nbad)/ n_elements(flux)
   ...
   g2 = where(xarr ge 15.14 and xarr le 15.18 and yarr ge 15.06 and yarr le 15.1, g2count);red
   g3 = where(xarr ge 15.06 and xarr le 15.1 and yarr ge 15.04 and yarr le 15.08, g3count)  ;blue
   g4 = where(xarr ge 15.1 and xarr le 15.14 and yarr ge 15.06 and yarr le 15.1, g4count)  ;cyan


;   print, 'narrow position', g1count
   if g1count gt 10 then begin
...
;------------------plot it
p1 = plot(bmjdg1arr - time0,fluxg1arr,  xtitle = 'Time(days)', ytitle = 'Normalized Mean Flux', '1s', sym_size = 0.5,   sym_filled = 1, xrange = [0,600], yrange = [0.99, 1.01], xthick = 2, ythick = 2) ;, errorbar_capsize = 0.005
...
result= MPFITFUN('linear',bmjdg1arr - time0,fluxg1arr, fluxerrg1arr, start, perror = perror, bestnorm = bestnorm,/quiet)    ;ICL
fp = plot(bmjdg1arr - time0, result(0)*(bmjdg1arr - time0) + result(1), $
   color = 'black', overplot = p1,thick = 2)
DOF     = N_ELEMENTS(bmjdg1arr) - N_ELEMENTS(result) ; deg of freedom
PCERROR = PERROR * SQRT(BESTNORM / DOF)     ; scaled uncertainties
print, 'black result', result, 'pcerror', pcerror, ' perror', perror
t1 = text(400, 0.999, sigfig(result(0), 3, /scientific) + '    ' + sigfig(pcerror(0), 3, /scientific), /data, color = 'black', font_style = 'bold')

result= MPFITFUN('linear',bmjdg2arr - time0,fluxg2arr, fluxerrg2arr, start, perror = perror, bestnorm = bestnorm,/quiet)    ;ICL
fp = plot(bmjdg2arr - time0, result(0)*(bmjdg2arr - time0) + result(1), $
   thick = 2, color = 'red', overplot = p1)
DOF     = N_ELEMENTS(bmjdg2arr) - N_ELEMENTS(result) ; deg of freedom
PCERROR = PERROR * SQRT(BESTNORM / DOF)     ; scaled uncertainties
print, '-------------------------'
print, 'red result', result, 'pcerror', pcerror, ' perror', perror
t1 = text(400, 0.997, sigfig(result(0), 3, /scientific) + '    ' + sigfig(pcerror(0), 3, /scientific), /data, color = 'red', font_style = 'bold')

result= MPFITFUN('linear',bmjdg3arr - time0,fluxg3arr, fluxerrg3arr, start, perror = perror, bestnorm = bestnorm,/quiet)    ;ICL
fp = plot(bmjdg3arr - time0, result(0)*(bmjdg3arr - time0) + result(1), $
   thick = 2, color = 'blue', overplot = p1)
DOF     = N_ELEMENTS(bmjdg3arr) - N_ELEMENTS(result) ; deg of freedom
PCERROR = PERROR * SQRT(BESTNORM / DOF)     ; scaled uncertainties
print, '-------------------------'
print, 'blue result', result, 'pcerror', pcerror, ' perror', perror
t1 = text(400, 0.995, sigfig(result(0), 3, /scientific) + '    ' + sigfig(pcerror(0), 3, /scientific), /data, color = 'blue', font_style = 'bold')

result= MPFITFUN('linear',bmjdg4arr - time0,fluxg4arr, fluxerrg4arr, start, perror = perror, bestnorm = bestnorm,/quiet)    ;ICL
fp = plot(bmjdg4arr - time0, result(0)*(bmjdg4arr - time0) + result(1), $
   thick = 2, color = 'cyan', overplot = p1)
DOF     = N_ELEMENTS(bmjdg4arr) - N_ELEMENTS(result) ; deg of freedom
PCERROR = PERROR * SQRT(BESTNORM / DOF)     ; scaled uncertainties
print, '-------------------------'
print, 'cyan result', result, 'pcerror', pcerror, ' perror', perror
t1 = text(400, 0.993, sigfig(result(0), 3, /scientific) + '    ' + sigfig(pcerror(0), 3, /scientific), /data, color = 'cyan', font_style = 'bold')
