...
dec_ref = -20.275561
  ...
  aorname = ['r45674240', 'r45674496'] ;ch2
  nfits = 130L*63L    
  print, 'nfits',nfits
  AORwasp16 = replicate({wasp16ob, ra:0D,dec:0D,xcen:dblarr(nfits),ycen:dblarr(nfits),flux:dblarr(nfits),fluxerr:dblarr(nfits), corrflux:dblarr(nfits), corrfluxerr:dblarr(nfits), sclktime_0:0D, timearr:dblarr(nfits),aor:' ', bmjdarr:dblarr(nfits), utcsarr:dblarr(nfits), bkgd:dblarr(nfits), bkgderr:dblarr(nfits)},n_elements(aorname))


  for a = 0, n_elements(aorname) - 1 do begin
     print, 'working on ',aorname(a)
     dir = '/Users/jkrick/irac_warm/pcrs_planets/wasp16/'+ string(aorname(a) ) 
     CD, dir                    ; change directories to the correct AOR directory
     command  =  "find ch2/bcd -name '*_bcd.fits' > /Users/jkrick/irac_warm/pcrs_planets/wasp16/bcdlist.txt"
     spawn, command
     command2 =  "find  ch2/bcd -name '*bunc.fits' > /Users/jkrick/irac_warm/pcrs_planets/wasp16/bunclist.txt"
     spawn, command2

     readcol,'/Users/jkrick/irac_warm/pcrs_planets/wasp16/bcdlist.txt',fitsname, format = 'A', /silent
     readcol,'/Users/jkrick/irac_warm/pcrs_planets/wasp16/bunclist.txt',buncname, format = 'A', /silent
...
;--------------------------------
 ...
 ;--------------------------------
    ...
    if n_elements(xarr) lt nfits then begin
       ...
       AORwasp16[a] ={wasp16ob,  ra_ref,dec_ref,xarr2,yarr2,fluxarr2,fluxerrarr2, corrfluxarr2, corrfluxerrarr2, sclk_0, timearr2,aorname(a), bmjd2,utcs2,back2, backerr2}
    endif else begin
       AORwasp16[a] ={wasp16ob,  ra_ref,dec_ref,xarr,yarr,fluxarr,fluxerrarr, corrfluxarr, corrfluxerrarr, sclk_0, timearr,aorname(a),bmjd, utcs,backarr, backerrarr}
    endelse

  endfor                        ;for each AOR
  
  ;test
  print, AORwasp16[0].ra, AORwasp16[0].xcen[26]
  
  save, AORwasp16, filename='/Users/jkrick/irac_warm/pcrs_planets/wasp16/wasp16_phot_ch2.sav'
  undefine, AORwasp16
