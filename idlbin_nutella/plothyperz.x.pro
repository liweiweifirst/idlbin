...
;fits_read, '/Users/jkrick/spitzer/mips/mips70/mosaic/Combine/mosaic.fits', mips70data, mips70header
;adxy, mips70header, objectnew[keeper].ra,objectnew[keeper].dec , mips70xcenter, mips70ycenter
fits_read, '/Users/jkrick/chandra/evt2/image_bin4.fits', xdata, xheader
adxy, xheader, objectnew[keeper].ra,objectnew[keeper].dec , xxcenter, xycenter
...
adxy,uheader, objectnew[keeper].ra,objectnew[keeper].dec , uxcenter,uycenter
...
;output the acs image of the objectnew
   ...
   if objectnew[keeper(n)].umag gt 0 then begin;and objectnew[keeper(n)].acsmag lt 90 
      plotimage, xrange=[uxcenter(n) - size/0.18,uxcenter(n)+ size/.18],$
                yrange=[uycenter(n) -size/.18,uycenter(n)+size/.18], $
                 bytscl(udata, min =-0.01, max = 0.05),title='u',$
...
;   if objectnew[keeper(n)].mips70mag gt 0 then begin ;and objectnew[keeper(n)].acsmag lt 90 
;      plotimage, xrange=[mips70xcenter(n) - size/2.5, mips70xcenter(n)+ size/2.5],$
;                 yrange=[mips70ycenter(n) -size/2.5, mips70ycenter(n)+size/2.5], $
;                 bytscl(mips70data, min = 15.45, max = 15.6),title='mips70',$
;                 /preserve_aspect, /noaxes, ncolors=60;,position=[0,0.5,0,0.5]
;;      xyouts, xcenter(n)- 0.6*size/0.05, -10., string(keeper(n)),charthick = 3
;   endif else begin
;      ;need a placeholder
;      plot, findgen(10), findgen(10), thick=0, xthick=0,ythick=0, charthick=0, charsize=0, xstyle=4, ystyle=4, psym=3
;      xyouts, 4, 4, 'mips70', charthick=3
;   endelse

   if objectnew[keeper(n)].xnetcounts gt 0 then begin ;and objectnew[keeper(n)].acsmag lt 90 
      plotimage, xrange=[xxcenter(n) - size/0.492, xxcenter(n)+ size/0.492],$
                 yrange=[xycenter(n) -size/0.492, xycenter(n)+size/0.492], $
                 bytscl(xdata, min =0.5, max = 25),title='chandra',$
                 /preserve_aspect, /noaxes, ncolors=60;,position=[0,0.5,0,0.5]
;      xyouts, xcenter(n)- 0.6*size/0.05, -10., string(keeper(n)),charthick = 3
   endif else begin
      ;need a placeholder
      plot, findgen(10), findgen(10), thick=0, xthick=0,ythick=0, charthick=0, charsize=0, xstyle=4, ystyle=4, psym=3
      xyouts, 4, 4, 'chandra', charthick=3
   endelse
