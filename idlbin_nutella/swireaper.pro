pro swireaper,image,xc,yc,mags,errap,sky,skyerr,phpadu,apr,skyrad,badpix, $
...
;       below BADLO threshold.  SKYBUF will be 1-d array of sky pixels
 ...
 if ( nsky LT minsky ) then begin                       ;Sufficient sky pixels?
  ...
  if keyword_set(meanback) then $
   meanclip,skybuf,skymod,skysig else $
...
;print,apmag
 ...
 good = where (apmag GT -999, Ngood)     ;Are there any valid integrated fluxes?
 ...
 nogood = where (apmag LE -999, Nbad) 
