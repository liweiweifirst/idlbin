pro plothyperz_noch34, keeper, filenameplot

print, '/Users/jkrick/zphot/zphot.noch34.param'
;print, "keeper", keeper
...
openw, outlunh, "/Users/jkrick/ZPHOT/hyperz_cat_noch34.txt",/get_lun
...
commandline = '~/bin/hyperz /Users/jkrick/ZPHOT/zphot.noch34.param'
spawn, commandline
...
readlargecol, '/Users/jkrick/ZPHOT/hyperz_swire_target.obs_sed',idz, u,g,r,i,acs,z,j,wj,tj,wh,th,wk,tk,one,two,$;three, four,$
         uerr, gerr, rerr, ierr,zerr,acserr, jerr, wjerr,tjerr,wherr,therr,wkerr,tkerr,oneerr, twoerr,format="A",/debug ;, threeerr, fourerr
...
;fits_read, '/Users/jkrick/palomar/wirc/coadd_k.fits',kdata,kheader
 ...
 ;plot the fitted SED along with photometry of an individual galaxy
  ...
  yerr = [uerr(n),gerr(n),rerr(n),ierr(n),acserr(n),zerr(n), jerr(n),wjerr(n),tjerr(n), wherr(n),therr(n),wkerr(n), tkerr(n),oneerr(n),twoerr(n)];,threeerr(n), fourerr(n)]
   ...
   plot,alog10(x), alog10(y), psym = 5, thick = 3, charthick = 3, xthick = 3, ythick = 3, $
        xtitle = "log(microns)", ytitle = "log(flux(microjansky))", xrange=[-0.5,1.5], title=strcompress("objectnew " + string(idz(n)) +string(zphota(n)) + string(proba(n)) + string(newsptkey(specta(n)-1)));,position=[0.5,1.0,0,0.5]
