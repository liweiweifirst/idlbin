...
for cand=0, n_elements(canddec) - 1 do begin
   ...
   good = where(sphdist(objectnew.ra, objectnew.dec, candra(cand), canddec(cand), /degrees) lt sep and objectnew.irac1mag gt 0 and objectnew.irac1mag lt 90 and objectnew.acsmag gt 0 and objectnew.acsmag lt 90) ;and objectnew.nearestdist*60*60/0.6 gt 4.0 
...
;;   plothist, objectnew[good].zphot, bin = 0.05, xrange=[0,1.5], /xstyle, thick = 3, xthick = 3, ythick = 3, charthick = 3, xtitle='Redshift', ystyle =1, ytitle='Number';title=strcompress(string(candra(cand)) + string(canddec(cand)))  yrange=[0,10],
...
;;   oplot, xvals, m*xvals + b(cand) + 0.5, thick = 3 , color = orangecolor
;;   oplot, xvals,  m*xvals + b(cand) - 0.5, thick = 3 , color = orangecolor
...
;---------------------------------------------------------------------
   ...
   for count = 0.D, n_elements(good) - 1 do begin
      limithigh = (m*objectnew[good[count]].irac1mag) + ball +0.5; 2*sig1;0.5 
      limitlow = (m*objectnew[good[count]].irac1mag) + ball - 0.5;2*sig1;0.5 
      ...
      if (objectnew[good[count]].acsmag - objectnew[good[count]].irac1mag LT limithigh ) AND ( objectnew[good[count]].acsmag - objectnew[good[count]].irac1mag GT limitlow) and objectnew[good[count]].irac1mag lt 24.6 and objectnew[good[count]].irac1mag gt 22.6 then dimcountclus = dimcountclus + 1
   ...
   print, 'cluster', singlecount, brightcountclus, dimcountclus
...
for cand=0, n_elements(randdec) - 1 do begin   ;for each random area
   ...
   for c =0, n_elements(good) -1 do begin
      ...
      limithigh = (m*objectnew[good[c]].irac1mag) + ball +0.5; 2*sig1;0.5 
      limitlow = (m*objectnew[good[c]].irac1mag) + ball - 0.5;2*sig1;0.5 
      ...
      if (objectnew[good[c]].acsmag - objectnew[good[c]].irac1mag LT limithigh )AND ( objectnew[good[c]].acsmag - objectnew[good[c]].irac1mag GT limitlow) then singlecountfield = singlecountfield + 1
   ...
   gc = gc + 1
   print, 'field ', singlecountfield, brightcount, dimcount
   singarr[cand] = singlecountfield

endfor 

free_lun, outlunback
; print, 'gooddimarr',gooddimarr
...
oplot, findgen(100),  -0.09*findgen(100) + 4.5, thick = 3 ;, color = orangecolor
oplot, findgen(100), -0.09*findgen(100) + 4.5 + 0.5, thick = 3 ;, color = orangecolor
oplot, findgen(100),  -0.09*findgen(100) +4.5 - 0.5, thick = 3 ;, color = orangecolor
...
plothist, objectnew[memberarr].irac1mag,  thick = 3, xthick = 3, ythick = 3, charthick = 3, $
          xtitle='[3.6] (AB)', ytitle='Number', xrange=[18,25], xstyle = 9, yrange=[0,40]
axis, 0,40, xaxis=1,xrange=[18-dm+kcorr,25-dm+kcorr], xstyle=1, xthick=3, charthick=3
oplot, [24.6, 24.6], [0,60], linestyle=2
oplot, [22.6, 22.6], [0,60], linestyle=2
oplot, [20.6, 20.6], [0,60], linestyle=2
...
rad = 0.05
field = where(sphdist(objectnew.ra, objectnew.dec, candra(0), canddec(0), /degrees)  gt rad and sphdist(objectnew.ra, objectnew.dec, candra(1), canddec(1), /degrees)  gt rad and sphdist(objectnew.ra, objectnew.dec, candra(2), canddec(2), /degrees)  gt rad and objectnew.irac1mag gt 0 and objectnew.irac1mag lt 90  and objectnew.acsmag gt 0 and objectnew.acsmag lt 90 and objectnew.acsmag - objectnew.irac1mag gt 2.0 and objectnew.acsmag - objectnew.irac1mag lt 3.5)
