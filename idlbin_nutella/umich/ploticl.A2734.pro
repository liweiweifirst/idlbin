...
YRANGE = [31, 20.0], xrange = [0,300],ytitle = out, $
...
;start=[0.02,100.0,0.5, 40.]
;result37 =  MPFITFUN('devauc2',radiusc,countsc, Verr, start)    ;ICL
...
;numbers = (0.43) * (exp(-7.67*(((findgen(700)/5.94)^(1.0/4.0)) - 1.0))) + (0.02) * (exp(-7.67*(((findgen(700)/141.1)^(1.0/4.0)) - 1.0)))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
OPENR, lun,'/Users/jkrick/umich/icl/A2734/gal2rtab', /GET_LUN
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;   add the cD galaxy
OPENR, lun,'/Users/jkrick/umich/icl/A2734/cdBtab2', /GET_LUN
...
radiusc = radiusc[0:j-1]
Verrc = Verrc[0:j-1]
oplot, radiusc* 0.435,22.19 - 2.5*alog10(countsc/(0.435^2)),thick = 3;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
OPENR, lun,'/Users/jkrick/umich/icl/A2734/icl2B2tab', /GET_LUN
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
OPENR, lun,'/Users/jkrick/umich/icl/A2734/icl2B3tab', /GET_LUN
...
lower2 = fltarr(n_elements(countsc))
FOR counter = 0, n_elements(countsc) - 1, 1 DO BEGIN
    IF (countsc(counter) - .00028 LE 0. )THEN BEGIN
        lower2(counter) = 35.
    ENDIF ELSE BEGIN
        lower2(counter) = 22.19 - 2.5*alog10((countsc(counter) - .00028)/(0.435^2))
    ENDELSE
ENDFOR

upper2 = 22.19 - 2.5*alog10((countsc + .00028)/(0.435^2)) 
errplot, radiusc* 0.435,lower2, upper2
...
;devauc fitting and plotting for cD inclusive
start = [0.03,200.0]
result2 = MPFITFUN('devauc',radiusc,countsc, Verrc, start)    ;ICL

numbers = (result2(0)) * (exp(-7.67*(((radiusc/result2(1))^(1.0/4.0)) - 1.0)))
oplot, radiusc*0.435, 22.19 - 2.5*alog10(numbers/(0.435^2)), thick = 5, color = colors.green;
...
numbers = (0.01) * (exp(-7.67*(((findgen(700)/76.)^(1.0/4.0)) - 1.0))) 
;oplot, findgen(700)*0.435, 22.04 - 2.5*alog10(numbers/(0.435^2)) , thick = 5, color = colors.purple

numbers = (0.05) * (exp(-7.67*(((findgen(700)/38.)^(1.0/4.0)) - 1.0))) 
;oplot, findgen(700)*0.435, 22.04 - 2.5*alog10(numbers/(0.435^2)) , thick = 5, color = colors.purple

numbers = (0.01) * (exp(-7.67*(((findgen(700)/46.)^(1.0/4.0)) - 1.0)))  +  (0.005) * (exp(-7.67*(((findgen(700)/308.)^(1.0/4.0)) - 1.0))) 
;oplot, findgen(700)*0.435, 22.04 - 2.5*alog10(numbers/(0.435^2)) , thick = 5, color = colors.pink



;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

OPENR, lun,'/Users/jkrick/umich/icl/A2734/gal2Btab', /GET_LUN
