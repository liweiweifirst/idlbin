...
openr, lun, "/n/Godiva6/jkrick/A3880/cmd/SExtractor.r.cat", /GET_LUN
...
openr, lun3, "/n/Godiva6/jkrick/A3880/cmd/2df.A3880.txt", /GET_LUN
...
xshift = 0.;-190
yshift = 0.;+60
openr, lun2, "/n/Godiva6/jkrick/A3880/cmd/SExtractor.B.cat", /GET_LUN
...
;    xyad,header,vx,vy,vra,vdec     ;find the ra and dec from the better astrometry image
    ...
    IF (vfwhm GT 4.2) AND (vfluxbest GT 0.0) AND (viso GT 6.0) THEN BEGIN ;not star,junk
...
;                IF (vmagbest - rgalaxy[j].magb GE 3.0)  THEN  $
...
FOR se = 0, m-1, 1 DO begin
    IF (mgalaxy[se].rmag LE 18.5) AND (mgalaxy[se].rmag GT 14.0)THEN BEGIN
        ...
        t = t +1
    ENDIF 
    IF (mgalaxy[se].rmag LT 20.) AND (mgalaxy[se].rmag GT 14.0) THEN BEGIN
...
;-----------------------------------------------------------
openw, nolun, "/n/Godiva6/jkrick/A3880/cmd/member.txt",/get_lun
memgalincut = 0.
nmemgalincut = 0.
memgalnincut = 0.
nmemgalnincut = 0.
...
FOR a = 0, m-1 ,1 DO BEGIN
    ...
    IF (mgalaxy[a].vr LT limithigh )AND (mgalaxy[a].vr GT limitlow) THEN BEGIN
       ;galaxy "a" makes the color cut
        ...
        dist1 = sqrt((mgalaxy[a].xcen- 1643.)^2 + (mgalaxy[a].ycen - 2278.)^2) 
;        dist2 = sqrt((mgalaxy[a].xcen-1157.)^2 + (mgalaxy[a].ycen - 2267.)^2) 

        IF dist1 LT 1255. THEN BEGIN
              rsum = rsum + 10^((22.04 - mgalaxy[a].rmag)/2.5)
              Vsum = Vsum + 10^((22.19 - (mgalaxy[a].vr + mgalaxy[a].rmag))/2.5)
          ENDIF
        
      IF dist1 LT 1635. AND mgalaxy[a].rmag LT 18.53 THEN density = density + 1
;        ENDIF
...
;    dist1 = sqrt((xarr(num)-1643.)^2 + (yarr(num) - 2278.)^2) 
;    IF (memarray(num) LT ( memarray[9] + 3)) AND (dist1 LE 1500.) THEN density = density + 1
;ENDFOR
...
device, filename = '/n/Godiva6/jkrick/A3880/cmd/cmd.ps', /portrait, $
  BITS=8, scale_factor=0.9 , /color
