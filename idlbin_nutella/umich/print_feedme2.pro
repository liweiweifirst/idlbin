FUNCTION print_feedme2, imagename, outputname,noiseimagename, psfname, segmentationname, isoarea, theta, a, b, xcenter, ycenter, mag, bkgd, nxaxis, nyaxis
...
ENDIF ELSE BEGIN
   ...
   ;figure out how large the fitting region should be, as per Peng's prescription
    ...
    xmin = fix(xcenter - xsize)
    xmax = fix(xcenter + xsize)
    ymin= fix(ycenter - ysize)
    ymax = fix(ycenter + ysize)
