...
;if (x GT  1510.03 ) and (x LT  1540.03 ) and (y gt  666.54 ) and (y lt 706.54 ) then flux = -100
    ...
    IF (isoarea GT 6) AND (flux GT 0) AND (mag LT 25.1) THEN BEGIN ;is a real object
;        print, "inside first if", x, y, mag
        IF ( e GE 0.1) OR (fwhm GT avfwhm+2*sig) THEN BEGIN ;is a galaxy
            IF (mag LT 22.5) AND (mag GE 22.1) AND (isorad GT 1.0)THEN begin
