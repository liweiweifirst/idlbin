...
FITS_READ, '/n/Godiva7/jkrick/A118/original/largeblank.fits', blankdata, blankheader

filename = "/n/Godiva7/jkrick/A118/original/largeV.div"
print, "working on", filename

imagefile = filename + '.fits'
;datafile = '/n/Godiva7/jkrick/A118/SExtractor.V.cat'
datafile = '/n/Godiva7/jkrick/A118/block_obj.r.input'
newimagefile = filename + '.s.fits'	
outfile = filename + '.bblock.fits'


;run SExtractor on the image to find the objects
commandline = '/n/Godiva7/jkrick/Sex/sex ' + imagefile + " -c /n/Godiva7/jkrick/A118/iclV.sex"
;spawn, commandline
...
satx = [721.706, 1615.319, 1332.976, 2004.792, 2142.377, 2240.587, 1225.326, 247.538, 1007.371, 1583.539, 1939.918, 1836.771  ]
saty = [ 2198.507, 2234.219, 1815.526, 1975.233, 1821.973, 1842.230, 956.661,642.119, 355.183,302.530,577.589, 236.343] 
satrmag=[15.9  , 14.2  , 14.4  , 15.5  , 14.5  , 15.8  , 11.9  , 15.4  , 14.7  , 15.2  , 14.9  , 13.5]  
satVmag = [16.5, 14.6, 14.9, 15.9, 14.8, 16.1, 12.1, 15.9, 15.2, 15.6, 15.5, 13.9]




;721.706    2198.507 15.9  16.5
;1615.319   2234.219 14.2  14.6
;1332.976   1815.526 14.4  14.9
;2004.792   1975.233 15.5  15.9
;2142.377   1821.973 14.5  14.8
;2240.587   1842.230 15.8  16.1
;1225.326    956.661 11.9  12.1
;247.538    642.119  15.4  15.9
;1007.371    355.183 14.7  15.2
;1583.539    302.530 15.2  15.6
;1939.918    577.589 14.9  15.5
;1836.771    236.343 13.5  13.9
...
;    print, "working on object", o
    ...
    IF (f LT 0) AND isoarea GT 50 THEN BEGIN    ;is a real object on top of the bad column
        ...
        return = del_star_A118(pa, f, sma , smb, xcenter, ycenter, data)
        true = 1
    ENDIF
    ...
    IF(isoarea LT 6.0 OR fwhm LT 3.9 AND true EQ 0) THEN BEGIN  ;2.5
        ;add 2 pixels to both a and b, and then block out to that radius
        ;print, "CR", o,xcenter,ycenter
        a = a + 2
        b = b + 2
       return = del_star_A118(pa, f, a , b, xcenter, ycenter, data)
...
;  IF (fwhm GE 2.8 AND fwhm LE 4.2 AND true EQ 0) THEN BEGIN    
    ...
    IF (fwhm GE 3.9 AND fwhm LE 4.5 AND true EQ 0) THEN BEGIN   
        ...
        IF (m GT 22.1) THEN BEGIN
            return = del_star_A118(pa, f, 8, 8, xcenter, ycenter, data)
            return = del_star_A118(pa, f, 8, 8, xcenter, ycenter, blankdata)
            true = 1
...
;            return = del_star_A118(pa, f, 22, 22, xcenter, ycenter, data)
           return = del_star_A118(pa, f, 30, 30, xcenter, ycenter, data)
           return = del_star_A118(pa, f, 30, 30, xcenter, ycenter, blankdata)
            true = 1
...
;                printf, starlun, "star in center with mag ", m
	...
	;seeing, focus, ...
            ...
            a= 30;19
            return = del_star_A118(pa, f, a , b, xcenter, ycenter, data)
            return = del_star_A118(pa, f, a , b, xcenter, ycenter, blankdata)
...
;    IF (fwhm GT 4.2 AND true EQ 0) THEN BEGIN
        IF (fwhm GT 4.5 AND true EQ 0) THEN BEGIN;5.1
 ...
 ;               print, "central galaxy bigger than 2000", xcenter, ycenter, isoarea
                sma = 1.4*sma ; 2.6,2.0,1.4                                 2.5;2.25*sma;2.0*sma;1.5*sma
                smb = 1.4*smb;  2.6,2.0,1.4                                2.5;2.25*smb;2.0*smb;1.5*smb
            ENDIF ELSE BEGIN

                sma = 1.6*sma; 3.0,2.3 ,1.6                     3.2;3.0*sma;2.6*sma;2*sma
                smb = 1.6*smb; 3.0,2.3,1.6                     3.2;3.0*smb;2.6*smb;2*smb
            ENDELSE
        ...
        theta = pa
;        return = del_star_A118(theta, f, sma , smb, xcenter, ycenter, data)
;        return = del_star_A118(pa, f, sma , smb, xcenter, ycenter, blankdata)
...
;them
j = "j"
OPENR, lunicl, "/n/Godiva7/jkrick/A118/iclV.sex", /GET_LUN
...
;fits_write, '/n/Godiva7/jkrick/A118/original/mask.block.galV.fits',shift(blankdata,-1,-1), blankheader

lessstars = data - blankdata   ;;;minus
;fits_write, '/n/Godiva7/jkrick/A114/galfit/nogals.s-stars.fits', shift(lessstars, -1, -1), header

fits_write,'/n/Godiva7/jkrick/A118/original/largeV.bblock.copy.fits', data, header
