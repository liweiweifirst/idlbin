...
openr, lun, "/n/Godiva4/jkrick/A2734/cmd/SExtractor.r.cat", /GET_LUN
WHILE (NOT eof(lun)) DO BEGIN
    READF,lun,junk, rx, ry, fluxaper,magaper,erraper,fluxbest,magbest,errbest,fwhm,rra,rdec,iso
    IF (fwhm GT 3.8) AND (fluxbest GT 0.0) AND (iso GT 6.0) THEN BEGIN   
...
openr, lun3, "/n/Godiva4/jkrick/A2734/cmd/collins.2734.member.txt", /GET_LUN
...
openr, lun3, "/n/Godiva4/jkrick/A2734/cmd/smith.member", /GET_LUN
WHILE (NOT eof(lun3)) DO BEGIN
...
openr, lun3, "/n/Godiva4/jkrick/A2734/cmd/smith.nonmember", /GET_LUN
WHILE (NOT eof(lun3)) DO BEGIN
...
openw, testlun, "/n/Godiva4/jkrick/A2734/cmd/2df.xy.txt", /get_lun
openr, lun3, "/n/Godiva4/jkrick/A2734/cmd/2df.txt", /GET_LUN
WHILE (NOT eof(lun3)) DO BEGIN
    READF, lun3,rh,rm,rs,dd,dm,ds,junk,z,junk
    r = (rh + ((rm + (rs / 60.)) / 60.))*15.
    d = dd - ((dm + (ds / 60.)) / 60.)
    adxy, header, r, d, xcen, ycen
    printf, testlun, xcen, ycen
    IF (z GT 0.0524) AND (z LT 0.08) THEN BEGIN ;member
        lgalaxy[l] = {couch,xcen,ycen}
        l = l+ 1
...
openr, lun3, "/n/Godiva4/jkrick/A2734/cmd/way.txt", /GET_LUN
WHILE (NOT eof(lun3)) DO BEGIN
    READF, lun3,r,d,cz,junk,junk,junk,junk
    precess, r, d, 1950.0, 2000.0
    adxy, header, r, d, xcen, ycen
    IF (cz GT 15720) AND (cz LT 24000) THEN BEGIN ;member
        lgalaxy[l] = {couch,xcen,ycen}
        l = l+ 1
    ENDIF ELSE BEGIN            ;nonmember
...
;;;                    ship = ship + 1
            ...
            IF j LT n  THEN BEGIN      ;is this v galaxy really a nonmember          
                IF (vx LT ngalaxy[j].x + 3.0) AND (vx GT ngalaxy[j].x - 3.0) AND $
                     (vy LT ngalaxy[j].y + 3.0) AND (vy GT ngalaxy[j].y - 3.0) THEN BEGIN
                    membership =  10.
;;                    ship = ship + 1
                ENDIF;;
...
FOR se = 0, m-1, 1 DO begin
    IF (mgalaxy[se].rmag LE 20.) AND (mgalaxy[se].rmag GT 15.0) $
      AND (mgalaxy[se].vr LT 2.5) AND (mgalaxy[se].vr GT 1.0) THEN BEGIN
        r1(t) = mgalaxy[se].rmag
        vr1(t) = mgalaxy[se].vr
        t = t +1
    ENDIF 
    IF (mgalaxy[se].rmag LT 22.) AND (mgalaxy[se].rmag GT 15.0) THEN BEGIN
...
;-----------------------------------------------------------
openw, nolun, "/n/Godiva4/jkrick/A2734/cmd/member.txt",/get_lun
memgalincut = 0.
nmemgalincut = 0.
memgalnincut = 0.
nmemgalnincut = 0.
...
FOR a = 0, m-1 ,1 DO BEGIN
    ...
    IF (mgalaxy[a].vr LT limithigh )AND (mgalaxy[a].vr GT limitlow) THEN BEGIN
       ;galaxy "a" makes the color cut
        ...
        dist1 = sqrt((mgalaxy[a].xcen-1530.)^2 + (mgalaxy[a].ycen - 1678.)^2) 
;        dist2 = sqrt((mgalaxy[a].xcen-1157.)^2 + (mgalaxy[a].ycen - 2267.)^2) 

        IF dist1 LE 1160. THEN BEGIN
            rsum = rsum + 10^((22.04 - mgalaxy[a].rmag)/2.5)
            Vsum = Vsum + 10^((22.19 - (mgalaxy[a].vr + mgalaxy[a].rmag))/2.5)
        ENDIF
        IF dist1 LE 1536. AND mgalaxy[a].rmag LT 18.68 THEN density = density + 1
;        ENDIF
...
;    dist1 = sqrt((xarr(num)-1530.)^2 + (yarr(num) - 1678.)^2) 
;    IF (memarray(num) LT ( memarray[9] + 3)) AND (dist1 LE 1385.) THEN density = density + 1
;ENDFOR

;print, "memarray", memarray
...
;;device, filename = '/n/Godiva4/jkrick/A2734/cmd/cmd.ps', /portrait, $
;;  BITS=8, scale_factor=0.9 , /color

plot,[0,0,0],[0,0,0],  thick = 3, psym = 2, xthick = 3, ythick = 3,charthick = 5,$
          xrange = [13,25], yrange=[0,3.5],ystyle = 1,xstyle = 1,$
...
print, "density", density - 2  ;get rid of first and second ranked galaxies
close, nolun
free_lun, nolun
END
