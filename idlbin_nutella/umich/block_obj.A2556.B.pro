...
;size of the image
xmax =1901;2866;1900;2866;2001;
ymax =2101;3486;2100;3486;2161;2900;
...
filename = "/n/Godiva3/jkrick/A2556/original/fullB"
...
;  IF (fwhm GE 2.8 AND fwhm LE 4.2 AND true EQ 0) THEN BEGIN    
    ...
    IF (fwhm GE 2.9 AND fwhm LE 4.4 AND true EQ 0) THEN BEGIN   
...
;           return = del_star_A2556(pa, f, 23, 23, xcenter, ycenter, blankdata)
            true = 1
...
;    IF (fwhm GT 4.2 AND true EQ 0) THEN BEGIN
    IF (fwhm GT 4.2 AND true EQ 0) THEN BEGIN ;5.1
        ...
        IF(isoarea LT 60) THEN BEGIN
                sma = 3.0*sma   ; 3.0,2.3 ,1.6                 
                smb = 3.0*smb   ; 3.0,2.3,1.6                  
        ENDIF         
        theta = pa
...
;fits_write, '/n/Godiva3/jkrick/A2556/original/mask.bblock.r.fits',shift(blankdata,-1,-1), blankheader

lessstars = data - blankdata   ;;;minus
;fits_write, '/n/Godiva3/jkrick/A2556/galfit/nogals.s-stars.fits', shift(lessstars, -1, -1), header

fits_write,'/n/Godiva3/jkrick/A2556/original/fullB.bblock.copy.fits', data, header
