...
cd, "/n/Truffle1/jkrick/galfit/sixmultiexp"
numogals = 6
...
FOR dist = 50,70, 2 DO BEGIN
   ...
   FOR k = 1, 20, 1 DO BEGIN
;dist = 0
;k = 0 
       spawn, "rm diagnostics.log"
...
;-----------------------------------------------------------------------------------        
        openw, outlun, "simulate.pro.feedme", /GET_LUN
        
        printf, outlun, "#IMAGE PARAMETERS"
        printf, outlun, "A) none" ;, "   # Input data image (FITS file)"
        printf, outlun, "B) fake.fits" ;, "     # Name for the output image"
        printf, outlun, "C) noise400.fits" ;, "   # Noise image name(made from data if blank or none) "
        ...
        openw, dlun3, "diagnostics.log", /GET_LUN, /append
        ...
        free_lun, outlun
        close, outlun
         spawn, "/n/pogo5/jess/galfit/galfit/galfit  simulate.pro.feedme"
        
        spawn, "rm galfit.??"
        
        fits_read, "fake.fits", fakedata, fakeheader
        fits_read, "noise400.fits", noisedata, noiseheader
        ...
        fits_write, "fake.fits", fakedata, fakeheader
       free_lun, dlun3
        close, dlun3
        
;        galfit , "/n/pogo5/jess/galfit/fake.fits", "/n/pogo5/jess/galfit/noise400.fits", "/n/pogo5/jess/galfit/psf.small.fits", "/n/pogo5/jess/galfit/segmentation.fits"
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
        ...
        commandline = '/n/Godiva7/jkrick/Sex/sex  fake.fits  -c  default.sex'
        spawn, commandline
...
;----------------------------------------------------------------------------------
        ...
        openw, outlun, "galfit2.pro.feedme", /GET_LUN
        
        ;need to change all of these names to real files 
        printf, outlun, "#IMAGE PARAMETERS"
        printf, outlun, "A) fake.fits" ;, "   # Input data image (FITS file)"
        printf, outlun, "B) junk.fits" ;, "     # Name for the output image"
        printf, outlun, "C) noise400.fits" ;, "   # Noise image nam
        printf, outlun, "D) psf.small.fits" ;,"            # Input PSF image 
;       printf, outlun, "E) ", "/n/pogo5/jess/galfit/multiexp/segmentation.fits";, "     # Pixel mask (
        printf, outlun, "F) /n/pogo5/jess/galfit/multiexp/const.test	       # Parameter constraint file (ASCII)"
        ...
        close, outlun
         ...
         ;run galfit
        spawn, '/n/pogo5/jess/galfit/galfit/galfit  galfit2.pro.feedme ', exit_status = y
...
;        IF (err NE 0) then PRINT, "galfit file did not open"
        string = " "
        xcentervals = fltarr(15)
        ycentervals = fltarr(15)
        magvals = fltarr(15)
        revals=fltarr(15)
        axisratiovals = fltarr(15)
        thetavals = fltarr(15)
        dsxvals = fltarr(15)
        dsyvals = fltarr(15)
...
;---------------------------------------------------------------------------------
        ...
        openw, outlun, "junk.feedme", /GET_LUN
        
        ;need to change all of these names to real files 
        printf, outlun, "#IMAGE PARAMETERS"
;        printf, outlun, "A) ","/n/pogo5/jess/galfit/multiexp/fake.fits" ;, "   # Input data image (FITS file)"
        printf, outlun, "B) gals.fits" ;, "     # Name for the output image"
        printf, outlun, "C) noise400.fits" ;, "   # Noise image nam
        printf, outlun, "D) psf.small.fits" ;,"            # Input PSF image 
;       printf, outlun, "E) ", "/n/pogo5/jess/galfit/multiexp/segmentation.fits";, "     # Pixel mask (
        printf, outlun, "F) const.test	       # Parameter constraint file (ASCII)"
...
;-------------------------------------------------------------------------------------------------------------------
        ...
        spawn, "/n/pogo5/jess/galfit/galfit/galfit junk.feedme"

        fits_read, "gals.fits", galsdata, galsheader
;        fits_read, "/n/pogo5/jess/galfit/multiexp/noise2.fits",noisedata, noiseheader
        fits_read, "fake.fits", fakedata, fakeheader
        residualdata = fakedata  -  galsdata 

        fits_write, strcompress("nogals" + string(k) + "." + string(dist)+ ".fits", $
                                /remove_all), residualdata, fakeheader
        newname = strcompress("diagnostics" + string(k)  + "." + string(dist) + ".log", /remove_all)
        newcommand = "mv diagnostics.log "+  newname
        ;newimg = strcompress("/n/pogo5/jess/galfit/multiexp/fake" + string(k) + "." + string(dist)+ ".fits", /remove_all)
        ;newcommand2 = "mv /n/pogo5/jess/galfit/multiexp/fake.fits " + newimg
        ;newimg2 = strcompress("/n/pogo5/jess/galfit/multiexp/gals" + string(k) + "." + string(dist)+ ".fits", /remove_all)
        ;newcommand3 = "mv /n/pogo5/jess/galfit/multiexp/junk.fits " + newimg2
