...
fits_read, "/n/Godiva7/jkrick/A118/original/larger.wcs.fits", data, header
...
openr, lun, "/n/Godiva7/jkrick/A118/cmd/SExtractor.r.cat", /GET_LUN
...
openr, lun3, "/n/Godiva7/jkrick/A118/astrometry/member.txt", /GET_LUN
WHILE (NOT eof(lun3)) DO BEGIN
    READF, lun3,rh,rm,rs,dd,dm,ds,z,z,z
    r = (rh + ((rm + (rs / 60.)) / 60.))*15.
    d = dd - ((dm + (ds / 60.)) / 60.)
...
;read in the non-membership information
numogals = 171
n = 0
ngalaxy = replicate({ncouch,x:0D,y:0D},numogals)
openr, lun3, "/n/Godiva7/jkrick/A118/astrometry/nonmember.txt", /GET_LUN
WHILE (NOT eof(lun3)) DO BEGIN
    READF, lun3,rh,rm,rs,dd,dm,ds,z,z,z
...
vra = 0D
vdec = 0D
openr, lun2, "/n/Godiva7/jkrick/A118/cmd/SExtractor.V.cat", /GET_LUN
openw, outlun, "/n/Godiva7/jkrick/A118/cmd/galaxies.txt", /get_lun
...
;    xyad,header,vx,vy,vra,vdec          ;find the ra and dec from the better astrometry image
    ...
    IF (vfwhm GT 4.9) AND (vfluxbest GT 0.0) AND (viso GT 6.0)THEN BEGIN ;not star or junk
...
;                    nship = nship + 1
            ...
            IF (vx LT rgalaxy[j].xcenter + 1.5 ) AND  (vx GT rgalaxy[j].xcenter - 1.5 ) AND $
                 (vy LT rgalaxy[j].ycenter + 1.5 ) AND (vy GT rgalaxy[j].ycenter - 1.5 )  AND $
                 (rgalaxy[j].isoarea GT 6.0) THEN BEGIN
                mgalaxy[m] = {what, vx,vy,vmagbest - rgalaxy[j].magb ,rgalaxy[j].magb,membership}

;                IF rgalaxy[j].magb LT 19.0 AND rgalaxy[j].magb GT 14.0 AND vmagbest - rgalaxy[j].magb LT 0.5  THEN  $
...
FOR se = 0, m-1, 1 DO begin
    IF (mgalaxy[se].rmag LE 23.0 )  THEN BEGIN
...
;-----------------------------------------------------------
openw, nolun, "/n/Godiva7/jkrick/A118/cmd/member.txt",/get_lun
openw, newlun, "/n/Godiva7/jkrick/A118/cmd/density.txt",/get_lun
memgalincut = 0.
nmemgalincut = 0.
memgalnincut = 0.
nmemgalnincut = 0.
...
;        IF mgalaxy[a].xcen LT 1600 AND mgalaxy[a].xcen GT 1250 AND mgalaxy[a].ycen LT 1680 AND mgalaxy[a].ycen GT 1230 THEN print, "galaxy makes cut", mgalaxy[a].xcen, mgalaxy[a].ycen
        ...
        dist1 = sqrt((mgalaxy[a].xcen- 1198.)^2 + (mgalaxy[a].ycen - 1184.)^2) 
;        dist2 = sqrt((mgalaxy[a].xcen-1157.)^2 + (mgalaxy[a].ycen - 2267.)^2) 

        IF dist1 LE 727. THEN BEGIN
...
;    dist1 = sqrt((xarr(num)-1198.)^2 + (yarr(num) - 1184.)^2) 
;    IF (memarray(num) LT ( memarray[9] + 3)) AND (dist1 LE 570.) THEN density = density + 1
;ENDFOR
;-------------------------------------------------------------
...
device, filename = '/n/Godiva7/jkrick/A118/cmd/junk.ps', /portrait, $
  BITS=8, scale_factor=0.9 , /color
