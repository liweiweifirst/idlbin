...
filename = "/n/Godiva7/jkrick/A118/original/larger.div"
;print, "working on", filename

imagefile = filename + '.fits'
datafile = '/n/Godiva7/jkrick/A118/SExtractor.r.cat'
...
commandline = '/n/Godiva7/jkrick/Sex/sex ' + imagefile + " -c /n/Godiva7/jkrick/A118/iclr.sex"
spawn, commandline
...
;    print, "working on object", o
    ...
    IF(isoarea LT 6.0 OR fwhm LT 5.5 AND true EQ 0) THEN BEGIN  ;2.5
...
;  IF (fwhm GE 2.8 AND fwhm LE 4.2 AND true EQ 0) THEN BEGIN    
    ...
    IF (fwhm GE 5.5 AND fwhm LE 6.5 AND true EQ 0) THEN BEGIN   
...
;    IF (fwhm GT 4.2 AND true EQ 0) THEN BEGIN
        IF (fwhm GT 6.5 AND true EQ 0) THEN BEGIN;5.1
 ...
 ;               print, "central galaxy bigger than 2000", xcenter, ycenter, isoarea
                    sma = ((1.4*sma)+0.93)/0.88 ; 2.6,2.0,1.4                 
                    smb =  ((1.4*smb)+0.93)/0.88 ;  2.6,2.0,1.4                 
                ENDIF ELSE BEGIN

                    sma =  ((1.6*sma)+0.93)/0.88 ; 3.0,2.3 ,1.6                 
                    smb =  ((1.6*smb)+0.93)/0.88 ; 3.0,2.3,1.6                  
                ENDELSE
        ...
        IF run EQ 1 THEN begin
            IF (sma GT 15.0 OR smb GT 15.0) THEN BEGIN ;big galaxies
                IF(isoarea GT 2000.) THEN BEGIN
 ;               print, "central galaxy bigger than 2000", xcenter, ycenter, isoarea
                    sma =  ((2.0*sma)+0.93)/0.88 ; 2.6,2.0,1.4                 
                    smb =  ((2.0*smb)+0.93)/0.88 ;  2.6,2.0,1.4                 
                ENDIF ELSE BEGIN

                    sma =  ((2.3*sma)+0.93)/0.88 ; 3.0,2.3 ,1.6                 
                    smb =  ((2.3*smb)+0.93)/0.88 ; 3.0,2.3,1.6                  
                ENDELSE
        ...
        IF run EQ 2 THEN begin
            IF (sma GT 15.0 OR smb GT 15.0) THEN BEGIN ;big galaxies
                IF(isoarea GT 2000.) THEN BEGIN
 ;               print, "central galaxy bigger than 2000", xcenter, ycenter, isoarea
                    sma =  ((2.6*sma)+0.93)/0.88 ; 2.6,2.0,1.4                 
                    smb =  ((2.6*smb)+0.93)/0.88 ;  2.6,2.0,1.4                 
                ENDIF ELSE BEGIN

                    sma =  ((3.0*sma)+0.93)/0.88 ; 3.0,2.3 ,1.6                 
                    smb =  ((3.0*smb)+0.93)/0.88 ; 3.0,2.3,1.6                  
                ENDELSE
...
;them
j = "j"
OPENR, lunicl, "/n/Godiva7/jkrick/A118/iclr.sex", /GET_LUN
...
if run EQ 0 THEN begin
fits_write, '/n/Godiva7/jkrick/A118/original/mask.block2.r.fits',shift(blankdata,-1,-1), blankheader
fits_write,'/n/Godiva7/jkrick/A118/original/larger.block2.fits', data, header
ENDIF
if run EQ 1 THEN begin
fits_write, '/n/Godiva7/jkrick/A118/original/mask.bblock2.r.fits',shift(blankdata,-1,-1), blankheader
fits_write,'/n/Godiva7/jkrick/A118/original/larger.bblock2.fits', data, header
ENDIF
if run EQ 2 THEN begin
fits_write, '/n/Godiva7/jkrick/A118/original/mask.vbblock2.r.fits',shift(blankdata,-1,-1), blankheader
fits_write,'/n/Godiva7/jkrick/A118/original/larger.vbblock2.fits', data, header
ENDIF
