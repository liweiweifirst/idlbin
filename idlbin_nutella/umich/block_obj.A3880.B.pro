...
FOR y = inty-maxradius, inty + maxradius,1 DO BEGIN
    FOR x = intx -maxradius, intx + maxradius,1 DO BEGIN
        ...
        IF (radius GE 45.0) THEN begin
            intensitysb = (3.9 + 2.5*alog10((43.5/radius)^(-1.6)) + sb2)
...
FITS_READ, '/n/Godiva6/jkrick/A3880/original/fullblank.fits', blankdata, blankheader

filename = "/n/Godiva6/jkrick/A3880/original/fullB"

imagefile = filename + '.fits'
datafile = '/n/Godiva3/jkrick/A2556/SExtractor.B.cat'
;datafile = '/n/Godiva1/jkrick/A4010/block_obj.r.input'
newimagefile = filename + '.s.fits'	
outfile = filename + '.bblock.fits'


;run SExtractor on the image to find the objects
commandline = '/n/Godiva7/jkrick/Sex/sex ' + imagefile + " -c /n/Godiva3/jkrick/A2556/iclB.sex"
spawn, commandline
...
satBmag = fltarr(200)
l = 0
OPENR, satlun, '/n/Godiva6/jkrick/A3880/satstar.txt', /GET_LUN
fits_read, '/n/Godiva6/jkrick/A3880/original/fullB.fits', datawcs, headerwcs
...
;      (satycen - 900. GT 0) AND (satycen - 900 LT 1801) $
    IF srmag LT 15.0 AND (satxcen  GT -300) AND (satxcen LT 3761) AND $
     (satycen  GT -300) AND (satycen LT 4891) THEN begin
        ...
        satBmag(l) = sbmag
        l = l +1
        print, satxcen, satycen, srmag, sBmag
    ENDIF
...
FOR satstar = 0, l-1, 1 DO BEGIN
    IF (satx[satstar] GT -300) AND (satx[satstar] LT 3490) AND $
      (saty[satstar] GT -300) AND (saty[satstar] LT 3950) THEN begin
;    holder = starmask (10^((satrmag[satstar] - 22.04)/(-2.5)), satx[satstar], saty[satstar],500.,xmax,ymax,blankdata)
        holder = starmask (10^((satBmag[satstar] - 22.04)/(-2.5)), satx[satstar], saty[satstar],500.,xmax,ymax,data)
...
;    print, "working on object", o
    ...
    IF(isoarea LT 6.0 OR fwhm LT 3.1 AND true EQ 0) THEN BEGIN  ;2.5
...
;  IF (fwhm GE 2.8 AND fwhm LE 4.2 AND true EQ 0) THEN BEGIN    
    ...
    IF (fwhm GE 3.1 AND fwhm LE 4.4 AND true EQ 0) THEN BEGIN   
...
;    IF (fwhm GT 4.2 AND true EQ 0) THEN BEGIN
 ...
 ;               print, "central galaxy bigger than 2000", xcenter, ycenter, isoarea
            ...
            ENDIF ELSE BEGIN
                ...
                sma = 1.6*sma   ; 3.0,2.3 ,1.6                 
                smb = 1.6*smb   ; 3.0,2.3,1.6                  
            ENDELSE
                
        ENDIF ELSE BEGIN        ;normal galaxies
            sma = 1.4*sma       ; 2.6,2.0,1.4                      
            smb = 1.4*smb       ; 2.6,2.0,1.4                      
        ENDELSE
...
;them
j = "j"
OPENR, lunicl, "/n/Godiva3/jkrick/A2556/iclB.sex", /GET_LUN
...
;fits_write, '/n/Godiva1/jkrick/A4059/original/mask.block.r.fits',shift(blankdata,-1,-1), blankheader

lessstars = data - blankdata   ;;;minus
;fits_write, '/n/Godiva1/jkrick/A4010/galfit/nogals.s-stars.fits', shift(lessstars, -1, -1), header

fits_write,'/n/Godiva6/jkrick/A3880/original/fullB.bblock.fits', data, header
