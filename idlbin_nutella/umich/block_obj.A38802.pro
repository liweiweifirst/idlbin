...
;size of the image
xmax =3461;1601;2866;2001;
ymax =4591;1801;3486;2161;2900;

;read in a blank image to which I can add my own stars			

FITS_READ, '/Users/jkrick/umich/icl/A3880/fullblank.fits', blankdata, blankheader

filename = "/Users/jkrick/umich/icl/A3880/fullrp"

imagefile = filename + '.fits'
datafile = '/Users/jkrick/umich/icl/A3880/SExtractor.r.cat'
...
satBmag = fltarr(105)
l = 0
OPENR, satlun, '/Users/jkrick/umich/icl/A3880/satstar.txt', /GET_LUN
fits_read, '/Users/jkrick/umich/icl/A3880/fullrp.fits', datawcs, headerwcs
satxcen = 0D
satycen = 0D
WHILE (NOT EOF(satlun)) DO BEGIN
    ...
    d = dd - ((dm + (ds / 60.)) / 60.);
;    print, "r, d", r, d
    adxy, headerwcs, r,d,satxcen, satycen,/print
;    print, satycen
...
;      (satycen - 900. GT 0) AND (satycen - 900 LT 1801) $
    IF srmag LT 15.0 AND (satxcen  GT -300) AND (satxcen LT 3761) AND $
     (satycen  GT -300) AND (satycen LT 4891) THEN begin
      ;AND 
        ...
        satBmag(l) = sbmag
        l = l +1
        print, satxcen, satycen, rh,rm,rs
    ENDIF

ENDWHILE
;print, "there are ", l, " satstars"
close, satlun
free_lun, satlun
satx = satx(0:l-1) ;- 700.
saty = saty(0:l-1) ;- 900.
...
;    print, "working on object", o
     ...
     IF xcenter LT 370 AND ycenter GT 3560 THEN true = 1  ;just junk in that corn  
    ...
    IF (f LT 0) AND isoarea GT 50 THEN BEGIN    ;is a real object on top of the bad column
        ...
        return = del_star_A3880(pa, f, sma , smb, xcenter, ycenter, data)
        true = 1
    ENDIF
    ...
    IF(isoarea LT 6.0 OR fwhm LT 3.2 AND true EQ 0) THEN BEGIN  ;2.5
        ;add 2 pixels to both a and b, and then block out to that radius
        ;print, "CR", o,xcenter,ycenter
        a = a + 2
        b = b + 2
       return = del_star_A3880(pa, f, a , b, xcenter, ycenter, data)
...
;  IF (fwhm GE 2.8 AND fwhm LE 4.2 AND true EQ 0) THEN BEGIN    
    ...
    IF (fwhm GE 3.2 AND fwhm LE 4.2 AND true EQ 0) THEN BEGIN   
        openw, dimlun, "/Users/jkrick/umich/icl/A3880/tvmark.out", /GET_LUN, /append
        ...
        IF (m GT 22.1) THEN BEGIN
            return = del_star_A3880(pa, f, 5, 5, xcenter, ycenter, data)
;            return = del_star_A3880(pa, f, 5, 5, xcenter, ycenter, blankdata)
            true = 1
...
;            return = del_star_A3880(pa, f, 22, 22, xcenter, ycenter, data)
           return = del_star_A3880(pa, f, 30, 30, xcenter, ycenter, data)
;           return = del_star_A3880(pa, f, 30, 30, xcenter, ycenter, blankdata)
            true = 1
...
;            holder = starmask (f, xcenter, ycenter, maxradius,xmax,ymax,blankdata)
	...
	;seeing, focus, ...
            ...
            a= 30;19
            return = del_star_A3880(pa, f, a , b, xcenter, ycenter, data)
;            return = del_star_A3880(pa, f, a , b, xcenter, ycenter, blankdata)
...
;    IF (fwhm GT 4.2 AND true EQ 0) THEN BEGIN
    IF (fwhm GT 4.2 AND true EQ 0) THEN BEGIN ;5.1
 ...
 ;               print, "central galaxy bigger than 2000", xcenter, ycenter, isoarea
                    sma = ((1.4*sma)+0.96)/0.870 ; 2.6,2.0,1.4                 
                    smb =  ((1.4*smb)+0.96)/0.870 ;  2.6,2.0,1.4                 
                ENDIF ELSE BEGIN

                    sma =  ((1.6*sma)+0.96)/0.870 ; 3.0,2.3 ,1.6                 
                    smb =  ((1.6*smb)+0.96)/0.870 ; 3.0,2.3,1.6                  
                ENDELSE
        ...
        IF run EQ 1 THEN begin
            IF (sma GT 15.0 OR smb GT 15.0) THEN BEGIN ;big galaxies
                IF(isoarea GT 2000.) THEN BEGIN
 ;               print, "central galaxy bigger than 2000", xcenter, ycenter, isoarea
                    sma =  ((2.0*sma)+0.96)/0.870 ; 2.6,2.0,1.4                 
                    smb =  ((2.0*smb)+0.96)/0.870 ;  2.6,2.0,1.4                 
                ENDIF ELSE BEGIN

                    sma =  ((2.3*sma)+0.96)/0.870 ; 3.0,2.3 ,1.6                 
                    smb =  ((2.3*smb)+0.96)/0.870 ; 3.0,2.3,1.6                  
                ENDELSE
        ...
        IF run EQ 2 THEN begin
            IF (sma GT 15.0 OR smb GT 15.0) THEN BEGIN ;big galaxies
                IF(isoarea GT 2000.) THEN BEGIN
 ;               print, "central galaxy bigger than 2000", xcenter, ycenter, isoarea
                    sma =  ((2.6*sma)+0.96)/0.870 ; 2.6,2.0,1.4                 
                    smb =  ((2.6*smb)+0.96)/0.870 ;  2.6,2.0,1.4                 
                ENDIF ELSE BEGIN

                    sma =  ((3.0*sma)+0.96)/0.870 ; 3.0,2.3 ,1.6                 
                    smb =  ((3.0*smb)+0.96)/0.870 ; 3.0,2.3,1.6                  
                ENDELSE
        ...
        theta = pa
        return = del_star_A3880(theta, f, sma , smb, xcenter, ycenter, data)
        return = del_star_A3880(pa, f, sma , smb, xcenter, ycenter, blankdata)
...
;            printf, gallun, xcenter, ycenter, sma, smb
        ...
        IF o EQ 4899 THEN print, "cD galaxy",sma, smb 
    ENDIF
...
IF run EQ 0 THEN begin
fits_write, '/Users/jkrick/umich/icl/A3880/mask.block2.rp.fits',shift(blankdata,-1,-1), blankheader
fits_write,'/Users/jkrick/umich/icl/A3880/fullrp.block2.fits', data, header
endif
IF run EQ 1 THEN begin
fits_write, '/Users/jkrick/umich/icl/A3880/mask.bblock2.rp.fits',shift(blankdata,-1,-1), blankheader
fits_write,'/Users/jkrick/umich/icl/A3880/fullrp.bblock2.fits', data, header
endif
IF run EQ 2 THEN begin
fits_write, '/Users/jkrick/umich/icl/A3880/mask.vbblock2.rp.fits',shift(blankdata,-1,-1), blankheader
fits_write,'/Users/jkrick/umich/icl/A3880/fullrp.vbblock2.fits', data, header
endif
