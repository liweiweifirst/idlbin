...
mgalaxy = replicate({what, xcen:0D,ycen:0D, vr:0D,rmag:0D, member:0D, fluxbe:0D},numoobjects)
...
;                  print,vx,vy
            ...
            IF (vx LT rgalaxy[j].xcenter + 0.6) AND  (vx GT rgalaxy[j].xcenter - 0.6) AND $
                ...
                mgalaxy[m] = {what, vx,vy,vmagbest - rgalaxy[j].magb ,rgalaxy[j].magb,membership, rgalaxy[j].fluxb}

                IF rgalaxy[j].magb LT 17.0 AND vmagbest - rgalaxy[j].magb LT 0.2 THEN  $
...
openw, memlun, "/n/Godiva1/jkrick/A3888/cmd/member.r.full.txt", /get_lun
memgalincut = 0.
nmemgalincut = 0.
memgalnincut = 0.
nmemgalnincut = 0.
...
FOR a = 0, m-1 ,1 DO BEGIN
    ...
    IF (mgalaxy[a].vr LT limithigh )AND (mgalaxy[a].vr GT limitlow) THEN BEGIN
       ...
       printf, memlun, mgalaxy[a].xcen, mgalaxy[a].ycen, mgalaxy[a].fluxbe
    ENDIF ELSE BEGIN
        IF (mgalaxy[a].member GT 0.) AND ( mgalaxy[a].member LT 9.)THEN BEGIN
...
device, filename = 'junk.ps', /portrait, $
  BITS=8, scale_factor=0.9 , /color
