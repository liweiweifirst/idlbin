...
close, /all
filename = strarr(1) 
;openr, lun5, "/n/Godiva4/jkrick/A141/original/listfull", /get_lun
...
FITS_READ, '/n/Godiva4/jkrick/A141/original/blank.fits', blankdata, blankheader

filename = "/n/Godiva4/jkrick/A141/original/larger"
;print, "working on", filename

imagefile = filename + '.fits'
datafile = '/n/Godiva4/jkrick/A141/SExtractor.r.cat'
;datafile = '/n/Godiva4/jkrick/A141/block_obj.r.input'
...
;V
;satx=[  35.90 , 429.75 ,2416.00 ,110.35  ,1040.71,1585.24 ]
;saty=[2567.90 , 2240.61 ,2468.90 ,198.58 , 521.35 ,55.07 ]
;satVmag=[15.7,16.4,13.6,14.9,15.7,13.8]
  
;r
;satx = [ 379.91, 2366.17 ,60.00,990.46 ,1534.96]
satx = [ 380.061, 2366.008   ,990.444 ,1535.006     ]
;saty = [2200.13, 2427.32 ,158.29   ,481.88  ,15.42]
saty = [2200.13,  2430.309 ,482.013  ,23.679]
;satrmag=[15.9,13.4,14.4,15.5,13.2]
satrmag=[15.9,13.4,15.5,13.2]

  

FOR satstar = 0, 3, 1 DO BEGIN
...
;    print, "working on object", o
    ...
    IF (f LT 0) AND isoarea GT 50 THEN BEGIN    ;is a real object on top of the bad column
        ...
        return = del_star_A141(pa, f, sma , smb, xcenter, ycenter, data)
        true = 1
    ENDIF
    ...
    IF(isoarea LT 6.0 OR fwhm LT 2.8 AND true EQ 0) THEN BEGIN  ;2.5
        ...
        a = a + 2
        b = b + 2
       return = del_star_A141(pa, f, a , b, xcenter, ycenter, data)
...
;  IF (fwhm GE 2.8 AND fwhm LE 4.2 AND true EQ 0) THEN BEGIN    
    ...
    IF (fwhm GE 2.8 AND fwhm LE 4.0 AND true EQ 0) THEN BEGIN   
        openw, dimlun, "/n/Godiva4/jkrick/A141/tvmark.out", /GET_LUN, /append
        ...
        IF (m GT 22.1) THEN BEGIN
            return = del_star_A141(pa, f, 8, 8, xcenter, ycenter, data)
            return = del_star_A141(pa, f, 8, 8, xcenter, ycenter, blankdata)
            true = 1
...
;            return = del_star_A141(pa, f, 22, 22, xcenter, ycenter, data)
           return = del_star_A141(pa, f, 30, 30, xcenter, ycenter, data)
           return = del_star_A141(pa, f, 30, 30, xcenter, ycenter, blankdata)
            true = 1
...
;                printf, starlun, "star in center with mag ", m
	...
	;seeing, focus, ...
            ...
            a= 30;19
            return = del_star_A141(pa, f, a , b, xcenter, ycenter, data)
            return = del_star_A141(pa, f, a , b, xcenter, ycenter, blankdata)
...
;    IF (fwhm GT 4.2 AND true EQ 0) THEN BEGIN
 ...
 ;               print, "central galaxy bigger than 2000", xcenter, ycenter, isoarea
                    sma = ((1.4*sma)+0.64)/0.94 ; 2.6,2.0,1.4                 
                    smb =  ((1.4*smb)+0.64)/0.94 ;  2.6,2.0,1.4                 
                ENDIF ELSE BEGIN

                    sma =  ((1.6*sma)+0.64)/0.94 ; 3.0,2.3 ,1.6                 
                    smb =  ((1.6*smb)+0.64)/0.94 ; 3.0,2.3,1.6                  
                ENDELSE
        ...
        IF run EQ 1 THEN begin
            IF (sma GT 15.0 OR smb GT 15.0) THEN BEGIN ;big galaxies
                IF(isoarea GT 2000.) THEN BEGIN
 ;               print, "central galaxy bigger than 2000", xcenter, ycenter, isoarea
                    sma =  ((2.0*sma)+0.64)/0.94 ; 2.6,2.0,1.4                 
                    smb =  ((2.0*smb)+0.64)/0.94 ;  2.6,2.0,1.4                 
                ENDIF ELSE BEGIN

                    sma =  ((2.3*sma)+0.64)/0.94 ; 3.0,2.3 ,1.6                 
                    smb =  ((2.3*smb)+0.64)/0.94 ; 3.0,2.3,1.6                  
                ENDELSE
        ...
        IF run EQ 2 THEN begin
            IF (sma GT 15.0 OR smb GT 15.0) THEN BEGIN ;big galaxies
                IF(isoarea GT 2000.) THEN BEGIN
 ;               print, "central galaxy bigger than 2000", xcenter, ycenter, isoarea
                    sma =  ((2.6*sma)+0.64)/0.94 ; 2.6,2.0,1.4                 
                    smb =  ((2.6*smb)+0.64)/0.94 ;  2.6,2.0,1.4                 
                ENDIF ELSE BEGIN

                    sma =  ((3.0*sma)+0.64)/0.94 ; 3.0,2.3 ,1.6                 
                    smb =  ((3.0*smb)+0.64)/0.94 ; 3.0,2.3,1.6                  
                ENDELSE
        ...
        theta = pa
        return = del_star_A141(theta, f, sma , smb, xcenter, ycenter, data)
        return = del_star_A141(pa, f, sma , smb, xcenter, ycenter, blankdata)

        ;openw, blocklun, "/n/Godiva4/jkrick/A141/galfit/block.log", /GET_LUN, /append
...
;them
j = "j"
OPENR, lunicl, "/n/Godiva4/jkrick/A141/iclr.sex", /GET_LUN
...
;fits_write, '/n/Godiva4/jkrick/A141/galfit/nogals.s-stars.fits', shift(lessstars, -1, -1), header

IF run EQ 0 THEN begin
fits_write, '/n/Godiva4/jkrick/A141/original/mask.block2.r.fits',shift(blankdata,-1,-1), blankheader
fits_write,'/n/Godiva4/jkrick/A141/original/larger.block2.fits', data, header
ENDIF
IF run EQ 1 THEN begin
fits_write, '/n/Godiva4/jkrick/A141/original/mask.bblock2.r.fits',shift(blankdata,-1,-1), blankheader
fits_write,'/n/Godiva4/jkrick/A141/original/larger.bblock2.fits', data, header
ENDIF
IF run EQ 2 THEN begin
fits_write, '/n/Godiva4/jkrick/A141/original/mask.vbblock2.r.fits',shift(blankdata,-1,-1), blankheader
fits_write,'/n/Godiva4/jkrick/A141/original/larger.vbblock2.fits', data, header
ENDIF

;make a profile of what is left
;random center choice
;result = funcprof("/n/Godiva4/jkrick/A141/galfit/nogals.s-stars", 492., 503.)
