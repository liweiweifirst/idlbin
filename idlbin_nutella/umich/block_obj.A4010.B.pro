...
sb4 = 22.19 - 2.5*alog10((D*mymoffat(15.4))/(0.435^2.))   ;sb @ 4arcsec
sb2 = 22.19 - 2.5*alog10((D*mymoffat(7))/(0.435^2.))      ;sb @ 2arcsec
...
FOR y = inty-maxradius, inty + maxradius,1 DO BEGIN
    FOR x = intx -maxradius, intx + maxradius,1 DO BEGIN
        ...
        IF radius LT 45.0 THEN BEGIN
            ...
            intensity = (0.435^2.)*(10.^((22.19-intensitysb)/(2.5)))
        ENDIF
...
;            intensitysb2 = (4.3 + 2.5*alog10((43.5/radius)^(-3.9)) + sb2)
            ...
            intensity = (0.435^2.)*(10.^((22.19-intensitysb)/(2.5))); + (0.435^2.)*(10.^((22.19-intensitysb2)/(2.5)))
        ENDIF
...
filename = "/n/Godiva4/jkrick/A4010/original/fullB"
...
;    holder = starmask (10^((satrmag[satstar] - 22.19)/(-2.5)), satx[satstar], saty[satstar],500.,xmax,ymax,blankdata)
    holder = starmask (10^((satBmag[satstar] - 22.19)/(-2.5)), satx[satstar], saty[satstar],500.,xmax,ymax,data)
...
;  IF (fwhm GE 2.8 AND fwhm LE 4.2 AND true EQ 0) THEN BEGIN    
    ...
    IF (fwhm GE 2.9 AND fwhm LE 4.0 AND true EQ 0) THEN BEGIN   
        ...
        IF (m GT 22.1) THEN BEGIN
            return = del_star(pa, f, 5, 5, xcenter, ycenter, data)
;            return = del_star(pa, f, 5, 5, xcenter, ycenter, blankdata)
            true = 1
...
;fits_write, '/n/Godiva4/jkrick/A4010/original/mask.bblock.full.r.fits',shift(blankdata,-1,-1), blankheader

lessstars = data - blankdata   ;;;minus
;fits_write, '/n/Godiva4/jkrick/A4010/galfit/nogals.s-stars.fits', shift(lessstars, -1, -1), header

fits_write,'/n/Godiva4/jkrick/A4010/original/fullB.bblock.fits', data, header
