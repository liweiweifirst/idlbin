...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
fits_read, "/n/Godiva3/jkrick/A2556/original/fullr.wcs.fits", data, header
...
openr, lun, "/n/Godiva3/jkrick/A2556/cmd/SExtractor.r.cat", /GET_LUN
WHILE (NOT eof(lun)) DO BEGIN
    READF,lun,junk, rx, ry, fluxaper,magaper,erraper,fluxbest,magbest,errbest,fwhm,rra,rdec,iso
    IF (fwhm GT 4.0) AND (fluxbest GT 0.0) AND (iso GT 6.0) THEN BEGIN   
...
openr, lun3, "/n/Godiva3/jkrick/A2556/cmd/smith.A2556.txt", /GET_LUN
WHILE (NOT eof(lun3)) DO BEGIN
    READF, lun3,r,d,mag,mag,mag,z,err
;    r = (rh + ((rm + (rs / 60.)) / 60.))*15.
;    d = dd - ((dm + (ds / 60.)) / 60.)
...
openr, lun3, "/n/Godiva3/jkrick/A2556/cmd/caretta.A2556.txt", /GET_LUN
WHILE (NOT eof(lun3)) DO BEGIN
    READF, lun3,junk,rh,rm,rs,dd,dm,ds,mag,mag,cz,err
    r = (rh + ((rm + (rs / 60.)) / 60.))*15.
    d = dd - ((dm + (ds / 60.)) / 60.)
    adxy, header, r, d, xcen, ycen
;    print, r, d, xcen,ycen
...
xshift = 0.;-190
yshift = 0.;+60
openr, lun2, "/n/Godiva3/jkrick/A2556/cmd/SExtractor.B.cat", /GET_LUN
...
;                    ship = ship + 1
 ...
 ;               IF rgalaxy[j].magb LT 22.0 AND $
 ;                 (vmagbest - rgalaxy[j].magb GT 3.0)  THEN  $
...
FOR se = 0, m-1, 1 DO begin
    IF (mgalaxy[se].rmag LE 19.) AND (mgalaxy[se].rmag GT 16.0)   THEN BEGIN
        r1(t) = mgalaxy[se].rmag
        vr1(t) = mgalaxy[se].vr
        t = t +1
    ENDIF 
    IF (mgalaxy[se].rmag LT 24.) AND (mgalaxy[se].rmag GT 16.0) THEN BEGIN
...
;-----------------------------------------------------------
openw, nolun, "/n/Godiva3/jkrick/A2556/cmd/member.txt",/get_lun
memgalincut = 0.
nmemgalincut = 0.
memgalnincut = 0.
nmemgalnincut = 0.
...
;        "galaxy makes cut", mgalaxy[a].xcen, mgalaxy[a].ycen
        ...
        ENDIF
        dist1 = sqrt((mgalaxy[a].xcen-1445.)^2 + (mgalaxy[a].ycen - 1605.)^2) 
        dist2 = sqrt((mgalaxy[a].xcen-1157.)^2 + (mgalaxy[a].ycen - 2267.)^2) 

        IF dist1 LE 845. OR dist2 LE 600. THEN BEGIN
            rsum = rsum + 10^((22.04 - mgalaxy[a].rmag)/2.5)
            Vsum = Vsum + 10^((22.19 - (mgalaxy[a].vr + mgalaxy[a].rmag))/2.5)
        ENDIF

        IF dist1 LE 1129 AND mgalaxy[a].rmag LT 19.46 THEN density = density + 1
;        ENDIF
...
;    dist1 = sqrt((xarr(num)-1445.)^2 + (yarr(num) - 1605.)^2) 
;    IF (memarray(num) LT ( memarray[9] + 3)) AND (dist1 LE 1035.) THEN density = density + 1
;ENDFOR
;-------------------------------------------------------------
...
device, filename = '/n/Godiva3/jkrick/A2556/cmd/junk.ps', /portrait, $
  BITS=8, scale_factor=0.9 , /color
