...
FOR y = inty-maxradius, inty + maxradius,1 DO BEGIN
    FOR x = intx -maxradius, intx + maxradius,1 DO BEGIN
        ...
        intensity = 0.
        IF radius LT 45.0 AND radius GT 0.0 THEN BEGIN
...
isoarea = 0.0D                  ;area inside of last isophote
...
;size of the image
xmax =2831;3190;3011;1601;2866;2001;
ymax =3271;3650;3461;1801;3486;2161;2900;

;read in a blank image to which I can add my own stars			

FITS_READ, '/n/Godiva1/jkrick/A4059/original/fullblank.fits', blankdata, blankheader

filename = "/n/Godiva1/jkrick/A4059/original/fullr"

imagefile = filename + '.fits'
datafile = '/n/Godiva3/jkrick/A2556/SExtractor.r.cat'
;datafile = '/n/Godiva1/jkrick/A4059/block_obj.r.input'
...
openw, testlun, "/n/Godiva1/jkrick/A4059/original/test.out",/get_lun
...
satBmag = fltarr(200)
l = 0
OPENR, satlun, '/n/Godiva1/jkrick/A4059/satstar.txt', /GET_LUN
fits_read, '/n/Godiva1/jkrick/A4059/original/fullr.wcs.fits', datawcs, headerwcs
WHILE (NOT EOF(satlun)) DO BEGIN
    readf, satlun, rh,rm,rs,dd,dm,ds,srmag,sbmag,junk,junk
    r = (rh + ((rm + (rs / 60.)) / 60.))*15.
    d = dd - ((dm + (ds / 60.)) / 60.);
    adxy, headerwcs, r, d, satxcen, satycen
    printf,testlun,"satstar", satxcen, satycen,srmag,sbmag
...
satx = satx(0:l-1) ;- 700.
saty = saty(0:l-1) ;- 900.
satrmag = satrmag(0:l-1)
satBmag = satBmag(0:l-1)

FOR satstar = 0, l-1, 1 DO BEGIN
    IF (satx[satstar] GT -300) AND (satx[satstar] LT 3100) AND $
      (saty[satstar] GT -300) AND (saty[satstar] LT 3570) THEN begin
...
WHILE (NOT EOF(lun)) DO BEGIN
    ...
    IF (f LT 0) AND isoarea GT 50 THEN BEGIN    ;is a real object on top of the bad column
        ...
        return = del_star_A4059(pa, f, sma , smb, xcenter, ycenter, data)
        true = 1
    ENDIF
    ...
    IF(isoarea LT 6.0 OR fwhm LE 2.3 AND true EQ 0) THEN BEGIN  ;2.5
        ;add 2 pixels to both a and b, and then block out to that radius
        ;print, "CR", o,xcenter,ycenter
        IF o EQ 4123 THEN print, "o is a cr",a,b
        a = a + 2.
        b = b + 2.
       return = del_star_A4059(pa, f, a , b, xcenter, ycenter, data)
...
;  IF (fwhm GE 2.8 AND fwhm LE 4.2 AND true EQ 0) THEN BEGIN    
    ...
    IF (fwhm GE 2.3) AND (fwhm LE 3.1) AND (true EQ 0) THEN BEGIN   
...
;        free_lun, dimlun
        ...
        IF (m GT 22.1) THEN BEGIN
            return = del_star_A4059(pa, f, 5, 5, xcenter, ycenter, data)
;            return = del_star_A4059(pa, f, 5, 5, xcenter, ycenter, blankdata)
            true = 1
...
;            return = del_star_A4059(pa, f, 22, 22, xcenter, ycenter, data)
           return = del_star_A4059(pa, f, 30, 30, xcenter, ycenter, data)
;           return = del_star_A4059(pa, f, 30, 30, xcenter, ycenter, blankdata)
            true = 1
...
;            holder = starmask (f, xcenter, ycenter, maxradius,xmax,ymax,blankdata)
	...
	;seeing, focus, ...
            ...
            a= 30;19
            return = del_star_A4059(pa, f, a , b, xcenter, ycenter, data)
;            return = del_star_A4059(pa, f, a , b, xcenter, ycenter, blankdata)
...
;    IF (fwhm GT 4.2 AND true EQ 0) THEN BEGIN
    IF (fwhm GT 3.1 AND true EQ 0) THEN BEGIN ;5.1
 ...
 ;               print, "central galaxy bigger than 2000", xcenter, ycenter, isoarea
                    sma = ((1.4*sma)+1.55)/0.898 ; 2.6,2.0,1.4                 
                    smb =  ((1.4*smb)+1.55)/0.898 ;  2.6,2.0,1.4                 
                ENDIF ELSE BEGIN

                    sma =  ((1.6*sma)+1.55)/0.898 ; 3.0,2.3 ,1.6                 
                    smb =  ((1.6*smb)+1.55)/0.898 ; 3.0,2.3,1.6                  
                ENDELSE
        ...
        IF run EQ 1 THEN begin
            IF (sma GT 15.0 OR smb GT 15.0) THEN BEGIN ;big galaxies
                IF(isoarea GT 2000.) THEN BEGIN
 ;               print, "central galaxy bigger than 2000", xcenter, ycenter, isoarea
                    sma =  ((2.0*sma)+1.55)/0.898 ; 2.6,2.0,1.4                 
                    smb =  ((2.0*smb)+1.55)/0.898 ;  2.6,2.0,1.4                 
                ENDIF ELSE BEGIN

                    sma =  ((2.3*sma)+1.55)/0.898 ; 3.0,2.3 ,1.6                 
                    smb =  ((2.3*smb)+1.55)/0.898 ; 3.0,2.3,1.6                  
                ENDELSE
        ...
        IF run EQ 2 THEN begin
            IF (sma GT 15.0 OR smb GT 15.0) THEN BEGIN ;big galaxies
                IF(isoarea GT 2000.) THEN BEGIN
 ;               print, "central galaxy bigger than 2000", xcenter, ycenter, isoarea
                    sma =  ((2.6*sma)+1.55)/0.898 ; 2.6,2.0,1.4                 
                    smb =  ((2.6*smb)+1.55)/0.898 ;  2.6,2.0,1.4                 
                ENDIF ELSE BEGIN

                    sma =  ((3.0*sma)+1.55)/0.898 ; 3.0,2.3 ,1.6                 
                    smb =  ((3.0*smb)+1.55)/0.898 ; 3.0,2.3,1.6                  
                ENDELSE
        ...
        theta = pa
        return = del_star_A4059(theta, f, sma , smb, xcenter, ycenter, data)
        return = del_star_A4059(pa, f, sma , smb, xcenter, ycenter, blankdata)
...
;        ENDIF
        ...
        dist = sqrt((860. - xcenter)^2 + (2021.-ycenter)^2)
        ...
        smaarr[smacount] = sma
        smacount = smacount + 1
;        IF o EQ 4123 THEN print, "o=4123 inside galaxies",sma,smb 
    ENDIF

    IF (true EQ 0) THEN printf, testlun,xcenter,ycenter,"did not get blocked"
...
IF run EQ 0 THEN begin
fits_write, '/n/Godiva1/jkrick/A4059/original/mask.block.fullr2.fits',shift(blankdata,-1,-1), blankheader
fits_write,'/n/Godiva1/jkrick/A4059/original/fullr.block.fits', data, header
print, "just wrote A4059 block"
ENDIF

IF run EQ 1 THEN begin
fits_write, '/n/Godiva1/jkrick/A4059/original/mask.bblock.fullr2.fits',shift(blankdata,-1,-1), blankheader
fits_write,'/n/Godiva1/jkrick/A4059/original/fullr.bblock.fits', data, header
print, "just wrote A4059 bblock"
ENDIF

IF run EQ 2 THEN begin
fits_write, '/n/Godiva1/jkrick/A4059/original/mask.vbblock.fullr2.fits',shift(blankdata,-1,-1), blankheader
fits_write,'/n/Godiva1/jkrick/A4059/original/fullr.vbblock.fits', data, header
print, "just wrote A4059 bblock"
ENDIF

;fits_write, '/n/Godiva1/jkrick/A4010/galfit/nogals.s-stars.fits', shift(lessstars, -1, -1), header

;make a profile of what is left
;random center choice
;result = funcprof("/n/Godiva1/jkrick/A4010/galfit/nogals.s-stars", 492., 503.)
...
device, filename = '/n/Godiva1/jkrick/A4059/masksizes.ps', /portrait, $
