...
!P.multi = [0,1,2]
outputfile = strcompress(dir_name + 'iwic210_noise.ps',/remove_all) ;'/Users/jkrick/nutella/IRAC/iwic210/iwic210_noise.ps'
...
;XXXfor pos = 0, 2 do begin ; for each position (3 positions)
...
for ch = 0, 1 do begin   ;for ch1 and ch2
   ...
   for exp = 0, n_elements(exptime) - 1 do begin
      print, '-----------'
      print, 'working on noise ch, exp ', ch+ 1, exptime
...
;listed these filenames. or else I cut and past his code into this one.
      if ch eq 0 then begin
         rawlist = strcompress(dir_name + 'rawlist_ch1_' + string(exptime(exp))+'s',/remove_all)
         bcdlist =  strcompress(dir_name + 'bcdlist_ch1_'+ string(exptime(exp))+'s',/remove_all)
      endif else begin
         rawlist = strcompress(dir_name + 'rawlist_ch2_'+ string(exptime(exp))+'s',/remove_all)
         bcdlist =  strcompress(dir_name + 'bcdlist_ch2_'+ string(exptime(exp))+'s',/remove_all)
      endelse
...
;apply the mask to the data
      for name = 0,  n_elements(rawimage) -1 do begin
         ...
         fits_read, dir_name+rawimage(name), rawdata_int, rawheader ;'/Users/jkrick/nutella/IRAC/iwic210/'
         fits_read, dir_name+bcdimage(name), bcddata, bcdheader ;'/Users/jkrick/nutella/IRAC/iwic210/'
