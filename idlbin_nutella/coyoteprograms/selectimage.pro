...
IF haveFits THEN BEGIN
   IF Keyword_Set(fits) THEN IF N_Elements(filter) EQ 0 THEN filter = ["*.fits", "*.fts"] ELSE $
      filter = [filter, "*.fits", "*.fts"]
ENDIF
...
IF N_Elements(filter) EQ 0 THEN BEGIN
   ...
   IF haveFits THEN filter = [filter, "*.fits", "*.fts"]
   allextensions = 1
...
IF haveFits THEN BEGIN
   button = Widget_Button(filterID, Value='FITS Files', UValue=['*.fits', '*.fts'], /Checked_Menu)
...
IF haveFits THEN BEGIN
   button = Widget_Button(filterID, Value='All Types', $
      UValue=['*.bmp', '*.dcm', '*.fits', '*.fts', '*.gif', '*.jpg', '*.jpeg', '*.jpe', '*.jpf', '*.jpx', $
