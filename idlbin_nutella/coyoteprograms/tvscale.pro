...
IF Keyword_Set(tvscl) THEN BEGIN
   ...
   ENDIF ELSE BEGIN
     IF N_Params() EQ 2 THEN BEGIN
         IF Keyword_Set(normal) THEN TV, BytScl(image, Top=!D.Table_Size-1, Max=max, Min=min), x, True=true, _Extra=extra, /Normal ELSE $
                                     TV, BytScl(image, Top=!D.Table_Size-1, Max=max, Min=min), x, True=true, _Extra=extra, /Device
     ENDIF
   ENDELSE
