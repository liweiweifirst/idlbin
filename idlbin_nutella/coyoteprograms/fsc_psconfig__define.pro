...
;   Fixed a problem with how fully-qualified filenames are parsed into DIRECTORY and FILENAME
;      variables when passed into program with the FILENAME keyword. 27 December 2005. DWF.
;   Added PAGETYPE and EUROPEAN fields to the keyword structure that is returned. These
;      keywords are ignored by the PS device, but they allow the user to save the keyword
;      structure and reuse it by keyword inheritance the next time the GUI is invoked. 17 July 2006. DWF.
;-
;###########################################################################
;
; LICENSE
...
; Must parse filename, possibly, if it is a fully-qualified filename.
IF N_Elements(filename) NE 0 THEN BEGIN
   f = FSC_Base_Filename(filename, Directory=d, Extension=e)
   IF e NE "" THEN BEGIN
      IF (f + '.' + e) NE filename THEN BEGIN
      filename = f + '.' + e
      IF N_Elements(directory) EQ 0 THEN directory = d
      ENDIF
   ENDIF ELSE BEGIN
      IF f NE filename THEN filename = f
   ENDELSE
ENDIF
