...
for ind = 0, n_elements(raarr) - 1 do begin
   ra = raarr(ind)
   dec = decarr(ind)
;for all the objects in the catalog
for i =0l, n_elements(object.ra) - 1 do begin
   ...
   if object[i].ra gt ra-deltara and  object[i].ra lt ra+ deltara then begin
      if object[i].dec GT dec-deltadec and  object[i].dec lt dec+deltadec then begin
         ...
         print, object[i].acsclassstar

         maglist= [object[i].umaga, object[i].gmaga, object[i].rmaga, object[i].imaga,  object[i].acsmag,  object[i].flamjmag, object[i].wircjmag,  object[i].wirchmag,  object[i].wirckmag,  object[i].irac1mag,object[i].irac2mag,object[i].irac3mag,object[i].irac4mag, object[i].mips24mag]
       ...
       ;first find which frame the object is on
         ...
         for j=0,n_elements(images)-1 do begin
            ...
            if ((x[0] LT hix) AND (x[0] GT lox) AND (y[0] LT hiy) AND (y[0] GT loy)) then begin
             ...
             ;     xyouts, x, y, '+', charthick = 3,alighnment = 0.5
               endif
