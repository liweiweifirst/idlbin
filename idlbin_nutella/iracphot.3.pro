...
;the point of this code is to use ACS priors on the location and shape
;of IRAC sources in order to do IRAC photometry below the IRAC
;confusion limit.

print, 'starting at '+systime() 

;make structures to keep the info about each channel together
...
;want to keep both old and new fluxes for IRAC
;move current irac1flux to irac1fluxold and put new flux into irac1flux

;make space for the old fluxes in the master catalog
...
;want to know which objects are stars
...
for ch = 0, 3 do begin   ;for each channel

   ;read in the images and coverage images
...
;by doing aperture photometry on the psf image
...
; convert that to IRAC mosaic units
            ap=ap*0.05/0.6
            semi_major=semi_major*0.05/0.6   ; hst pixel size = 0.05", IRAC pixel size = .6"
            semi_minor=semi_minor*0.05/0.6
; now set the minimum aperture
            if ap lt 3.2 then ap =3.2

; this fixes the axis ratio value to give a minimum semi_minor axis
