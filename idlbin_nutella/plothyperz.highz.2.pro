pro plothyperz_highz, keeper, filenameplot
...
;                 objectnew[keeper(num)].tmassjmag,  objectnew[keeper(num)].swircjmagauto, wirchmagab, objectnew[keeper(num)].tmasshmag, wirckmagab, $
...
;                 objectnew[keeper(num)].wircjmagerr,tmassjerr, objectnew[keeper(num)].swircjmagautoerr,objectnew[keeper(num)].wirchmagerr, $
;                 tmassherr, objectnew[keeper(num)].wirckmagerr,$
...
;                 objectnew[keeper(num)].irac3magerr, objectnew[keeper(num)].irac4magerr;err1,err2,err3,err4;
  ...
  printf, outlunh, format='(I10,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2,F10.2, F10.2, F10.2)',$
                 ...
                 objectnew[keeper(num)].wircjmagerr,tmassjerr, objectnew[keeper(num)].swircjmagautoerr,objectnew[keeper(num)].wirchmagerr, tmassherr, objectnew[keeper(num)].wirckmagerr,$
                 tmasskerr,objectnew[keeper(num)].irac1magerr,objectnew[keeper(num)].irac2magerr,$
                 objectnew[keeper(num)].irac3magerr, objectnew[keeper(num)].irac4magerr;err1,err2,err3,err4;
...
;      xyouts, 0, -10., strcompress( string(objectnew[keeper(n)].ra) +'   '+ string(objectnew[keeper(n)].dec)),charthick = 3
   ...
   if objectnew[keeper(n)].swircjmagauto gt 0 then begin;and objectnew[keeper(n)].acsmag lt 90 
      plotimage, xrange=[jxcenter(n) - size/0.15,jxcenter(n)+ size/.15],$
                 yrange=[jycenter(n) -size/.15,jycenter(n)+size/.15], $
                 bytscl(jdata, min =-43, max = 43),title='j',$;
                 /preserve_aspect, /noaxes, ncolors=60
;      xyouts, 0, -10., strcompress( string(objectnew[keeper(n)].ra) +'   '+ string(objectnew[keeper(n)].dec)),charthick = 3
   endif else begin
      ;need a placeholder
      plot, findgen(10), findgen(10), thick=0, xthick=0,ythick=0, charthick=0, charsize=0, xstyle=4, ystyle=4, psym=3
      xyouts, 4, 4, 'j, charthick=3
   endelse
