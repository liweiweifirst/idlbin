...
;  fits_read, '/Users/jkrick/idlbin/pmap_fits/pmap_ch2_0p1s_x4_500x500_0043_120124.fits', pmapdata, pmapheader
  ...
  restore, '/Users/jkrick/irac_warm/pcrs_planets/wasp15/wasp15_phot_ch2.sav'
  aorname = ['r45675264', 'r45675776'] ;ch2


;-------------------------------------------------------
;taken from nsted planet properties calculator XXX
...
;set phase = 0.0 in the middle of the transit
  for a = 0, n_elements(aorname) - 1 do  begin
      AORwasp15[a].bmjdarr = ((AORwasp15[a].bmjdarr - utmjd_center) / period) 
   endfor

print, 'phase', AORwasp15[0].bmjdarr[0:100]


;  fits_read, '/Users/jkrick/idlbin/pmap_fits/pmap_ch2_0p1s_x4_500x500_0043_120124.fits', pmapdata, pmapheader
;  c = contour(pmapdata, /fill, n_levels = 21, rgb_table = 0, xtitle = 'X (pixel)', ytitle = 'Y (pixel)', aspect_ratio = 1, xrange = [0,500], yrange = [0,500])
;  for a = 0, n_elements(aorname) - 1 do begin
;     xcen500 = 500.* (AORwasp15[a].xcen - 14.5)
;     ycen500 = 500.* (AORwasp15[a].ycen - 14.5)
...
;        am = plot( (AORwasp15[a].timearr - AORwasp15[0].timearr(0))/60./60., AORwasp15[a].xcen,'6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a], xtitle = 'Time(hrs)', ytitle = 'X pix', yrange = [14.8, 15.3], xrange = [0,10])
;     endif else begin
;        am = plot( (AORwasp15[a].timearr - AORwasp15[0].timearr(0))/60./60., AORwasp15[a].xcen,'6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a],/overplot)
;     endelse
...
;  for a = 0, n_elements(aorname) - 1 do begin
;     if a eq 0 then begin
;        am = plot( (AORwasp15[a].timearr - AORwasp15[0].timearr(0))/60./60., AORwasp15[a].ycen,'6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a], xtitle = 'Time(hrs)', ytitle = 'Y pix',  yrange = [14.8, 15.3], xrange = [0,10])
;     endif else begin
;        am = plot( (AORwasp15[a].timearr - AORwasp15[0].timearr(0))/60./60., AORwasp15[a].ycen,'6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a],/overplot)
;     endelse
...
;  for a = 0, n_elements(aorname) - 1 do begin
;     if a eq 0 then begin
;        am = plot( (AORwasp15[a].timearr - AORwasp15[0].timearr(0))/60./60., AORwasp15[a].bkgd,'6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a], xtitle = 'Time(hrs)', ytitle = 'Background', xrange = [0,10])
;     endif else begin
;        am = plot( (AORwasp15[a].timearr - AORwasp15[0].timearr(0))/60./60., AORwasp15[a].bkgd,'6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a],/overplot)
;     endelse
...
;  for a = 0, n_elements(aorname) - 1 do begin
;     if a eq 0 then begin
;        am = errorplot( (AORwasp15[a].timearr - AORwasp15[0].timearr(0))/60./60., AORwasp15[a].flux,AORwasp15[a].fluxerr, '6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a], xtitle = 'Time(hrs)', ytitle = 'Flux', xrange = [0,10], yrange = [0.03,0.045], errorbar_capsize = 0.)
;        am = errorplot( (AORwasp15[a].timearr - AORwasp15[0].timearr(0))/60./60., AORwasp15[a].corrflux-0.003,AORwasp15[a].corrfluxerr,'6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a], /overplot, errorbar_capsize = 0.)
;     endif else begin
;        am = errorplot( (AORwasp15[a].timearr - AORwasp15[0].timearr(0))/60./60., AORwasp15[a].flux,AORwasp15[a].fluxerr,'6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a],/overplot, errorbar_capsize = 0.)
; am = errorplot( (AORwasp15[a].timearr - AORwasp15[0].timearr(0))/60./60., AORwasp15[a].corrflux-0.003,AORwasp15[a].corrfluxerr,'6r1s', sym_size = 0.1,   sym_filled = 1, color = colorarr[a],/overplot, errorbar_capsize = 0.)
;     endelse
...
;------
;a = where(finite(AORwasp15[1].corrflux) gt 0 and finite(AORwasp15[1].flux) gt 0)

; print, (abs(AORwasp15[1].corrflux[a[0:100]] - AORwasp15[1].flux[a[0:100]])/ AORwasp15[1].flux[a[0:100]])*100
;plothist,(abs(AORwasp15[1].corrflux[a[0:100]] - AORwasp15[1].flux[a[0:100]])/ AORwasp15[1].flux[a[0:100]])*100, xhist, yhist, bin = 0.1,/fill


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;try this diffferently, try putting the AORs togetether as one array; then bin

;timearr = [AORwasp15[0].timearr, AORwasp15[1].timearr]
;fluxarr = [AORwasp15[0].flux, AORwasp15[1].flux]
;fluxerrarr = [AORwasp15[0].fluxerr, AORwasp15[1].fluxerr]
;corrfluxarr = [AORwasp15[0].corrflux, AORwasp15[1].corrflux]
;corrfluxerrarr = [AORwasp15[0].corrfluxerr, AORwasp15[1].corrfluxerr]
;xarr = [AORwasp15[0].xcen, AORwasp15[1].xcen]
;yarr = [AORwasp15[0].ycen, AORwasp15[1].ycen]
;bkgd = [AORwasp15[0].bkgd, AORwasp15[1].bkgd]
;bmjd = [AORwasp15[0].bmjdarr, AORwasp15[1].bmjdarr]

;actually don't include the initial 30min AOR.
timearr = [AORwasp15[1].timearr]
fluxarr = [ AORwasp15[1].flux]
fluxerrarr = [ AORwasp15[1].fluxerr]
corrfluxarr = [ AORwasp15[1].corrflux]
corrfluxerrarr = [AORwasp15[1].corrfluxerr]
xarr = [ AORwasp15[1].xcen]
yarr = [AORwasp15[1].ycen]
bkgd = [ AORwasp15[1].bkgd]
bmjd = [ AORwasp15[1].bmjdarr]

print, 'n', n_elements(timearr), n_elements(AORwasp15[0].timearr), n_elements(AORwasp15[1].timearr)
;now for the binning
...
;     pl = errorplot((bin_timearr - bin_timearr(0))/60./60., bin_flux/median(bin_flux),bin_fluxerr, '6r1s', sym_size = 0.1,   sym_filled = 1, xrange = [0,9],  yrange = [0.99, 1.02], xtitle = 'Time (hrs)', ytitle = 'Normalized Flux', errorbar_capsize = 0.)
      ...
      pl = plot((bin_timearr - bin_timearr(0))/60./60., bin_flux/median(bin_flux), '6r1s', sym_size = 0.1,   sym_filled = 1,  xrange = [0,9],yrange = [0.98, 1.01], xtitle = 'Time (hrs)', ytitle = 'Normalized Flux', title = 'Wasp 15')
