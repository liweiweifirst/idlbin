...
E_sec=2.d0*atan(sqrt(1d0-e1^2)*tan(.5d0*f_sec),1d0+e1)
...
pro driver_trans_sec_phase_ramp, channel, phase_func, ramp_flag, cowan_opt, sfile

COMMON data, bjd_tot, flux_tot, nbr_ind, gw, err_tot, time_tot, nonlin_c

if (channel eq 1) then begin
restore,'ch1_filt_nbr_npapr_6.sav'
;Teff=6429, log=3.950, Fe/H=-0.177
nonlin_c=[-0.0132001,     0.796451,    -0.866651,     0.304464]
Tc=1396.70071d0
err_tot=err_tot*1.25d0
p0_test=[3.19154d0, 79.3d0*!dtor, 4.8d0, 0.280195, -0.0683042, Tc,  0.09455162d0, 0.001010d0, 0.001010d0]
endif

if (channel eq 2) then begin
;restore,'ch2_filt_nbr_fixapr_5.sav'
restore,'ch2_filt_nbr_fixapr_5_1dtrim.sav'
nonlin_c=[0.0265720,     0.384472,    -0.280933,    0.0548588]
Tc=1419.04141d0
p0_test=[3.19154d0, 79.3d0*!dtor, 4.8d0, 0.280195, -0.0683042, Tc,  0.09455162d0, 0.001430d0, 0.001430d0]
endif


p0_ramp=[0.0016d0, 0.07d0]
;initial fit from models
p0_phase=[[0.0008d0, 1.0d0, 0.5d0, 0.5d0, 0.0001d0],$ ;flat
...
p = mpfit('calc_trans_sec_phase', parinfo=pa, functargs=fa, perror=perror, status=status, bestnorm=bestnorm, ERRMSG=ERRMSG, /fastnorm)
