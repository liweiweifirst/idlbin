...
;dir_name = '/Users/jkrick/iwic/iwic_recovery1/IRAC017000'
...
;figure out which bias we are using the hard way
      if ch eq 0 then biasarr = biasarr_ch1
      if ch eq 1 then biasarr = biasarr_ch2

      bias = biasarr(aor)

      if ch eq 0 then bias_ch1(aor) = bias
      if ch eq 1 then bias_ch2(aor) = bias
...
;            title = "channel "+string( ch+1)+ "    exptime " + string(exptime(exp)), thick=3, xthick=3,ythick=3,charthick=3
         ...
         plot, findgen(n_elements(rawimage)), med_4(0,*),  psym = 1, xtitle = 'frame number', $
               ...
               title = "channel "+string( ch+1)+"  AFPAT2B" + string(AFPAT2B)+ " bias " + string(bias) ,  thick=3, $
...
;now plot the histogram of stddevs of all unmasked pixels
         plothist, sigmaarr, xjunk, yjunk, bin = binsize, xtitle = 'Standard deviation', ytitle='Number', $
                   ...
                   title = "channel "+ string(ch+1)+  "  AFPAT2B" + string(AFPAT2B)  + " bias " + string(bias),$ 
