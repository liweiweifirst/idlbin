pro virgo_m87_aper
     nan = alog10(-1)
     fits_read,  '/Users/jkrick/virgo/m87/same_fif/ch1/ch1_Combine-mosaic/mosaic.fits', data, header
     fits_read, '/Users/jkrick/virgo/m87/same_fif/ch1/ch1_Combine-mosaic/mosaic_cov.fits', covdata, covheader
     fits_read, '/Users/jkrick/Virgo/m87/same_fif/ch1_background.fits', backdata, backheader
...
;read in the locations of the globulars
    ...
    ; adxy, header, ra_start, dec_start, x_start, y_start
  filename ='/Users/jkrick/Virgo/m87/NGVS.ptcat.ugiz.i24.5.v0.2.pps99.m87.fits.gz'
     ngvs = mrdfits( filename,1,ngvsheader)
...
;array location dependent correction
     fits_read, '/Users/jkrick/Virgo/m87/same_fif/ch1/ch1_Combine-mosaic/mosaic_arraycorr.fits', arraydata, arrayheader
     ...
     nannum = where(finite(flux) ne 1, nancount)
     print, 'testing after arraycorr', nancount
...
;need to do ch2 = 4.5 microns
   ...
   ;simple aperture photometry on ch2
     ...
     fits_read,  '/Users/jkrick/Virgo/m87/same_fif/ch2/ch1_Combine-mosaic/mosaic.fits', data_ch2, header_ch2
     fits_read,  '/Users/jkrick/Virgo/m87/same_fif/ch2/ch1_Combine-mosaic/mosaic_cov.fits', covdata_ch2, covheader_ch2
...
;array location dependent correction
     fits_read, '/Users/jkrick/Virgo/m87/same_fif/ch2/ch1_Combine-mosaic/mosaic_arraycorr.fits', arraydata, arrayheader
     array_corr = arraydata[gclus.ch1_xcen, gclus.ch1_ycen]
...
;need some sort of useful output file
  ...
  ;   endfor
     ...
     save, gclus, filename = '/Users/jkrick/Virgo/m87/gclus.sav'

;also write this out in a fits file
     mwrfits, gclus, '/Users/jkrick/Virgo/m87/ngvs_irac.fits'



;make a region file to visually check the centroiding
     openw, outlun, '/Users/jkrick/Virgo/m87/irac_ra_dec_g24.reg', /get_lun
     for r = 0L,  n_elements(gclus.ch1_ra) - 1 do begin
        if finite(gclus(r).ch1_ra) lt 1 then gclus(r).ch1_ra = 0.0
        if finite(gclus(r).ch1_dec) lt 1 then gclus(r).ch1_dec = 0.0
        if gclus(r).flag_confuse eq 1 then gclus(r).ch1_ra = 0.0
        printf, outlun, format ='(A, g10.9, A, g10.9, A)', 'J2000; circle  ',ngvs[r].alpha_j2000, 'd  ', ngvs[r].delta_j2000,    'd 5.0" # color=green'
        printf, outlun,format ='(A, g10.9, A, g10.9, A)', 'J2000; circle  ', gclus(r).ch1_ra, 'd  ', gclus(r).ch1_dec,    'd 4.5" # color=red'
     endfor
     close, outlun
     free_lun, outlun
