pro wasp14_selfcal_ch2, binning=binning, sine=sine
...
;really the jumps in pcrspeakup observing to move back to sweet spot
 ...
 starttransit = max(where(timearr lt 45.0)) ;3.0
  stoptransit = min(where(timearr gt 65.)) ;6.5
  xarr = [xarr[0:starttransit],xarr[stoptransit:n_elements(timearr)-1]]
  yarr = [yarr[0:starttransit],yarr[stoptransit:n_elements(timearr)-1]]
  fluxerr = [fluxerr[0:starttransit],fluxerr[stoptransit:n_elements(timearr)-1]]
  flux = [flux[0:starttransit],flux[stoptransit:n_elements(timearr)-1]]
...
;     st2 = plot(bin_time, bin_model_y+14.5,'1sb',/overplot, sym_size = 0.2, sym_filled = 1.)
  ...
  ;x vs y
   ...
   ;-----------------
     ...
     p1 = plot(x, y,  '1s',  xtitle = 'Time(hrs)', ytitle = ' Flux',  sym_size = 0.2, sym_filled = 1.,title = titlename, yrange = [0.0605, 0.062]) ;, xrange = [2,4]
     f1 = plot(x,bin_model,  '1sb',/overplot , sym_size = 0.2, sym_filled = 1.) ;-0.0027
     f2 = plot(x, bin_sub+0.061, '1sr',/overplot , sym_size = 0.2, sym_filled = 1.)
  ...
  save, selfcal_timearr, filename='/Users/jkrick/irac_warm/pcrs_planets/wasp14/selfcal_timearr.sav'
  save, selfcal_flux, filename='/Users/jkrick/irac_warm/pcrs_planets/wasp14/selfcal_flux.sav'
  save, bin_time, filename='/Users/jkrick/irac_warm/pcrs_planets/wasp14/bin_time.sav'
  save, bin_sub, filename='/Users/jkrick/irac_warm/pcrs_planets/wasp14/bin_sub.sav'
