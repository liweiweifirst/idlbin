...
;ps_start, filename='/Users/jkrick/iwic/start_science2.ps'
...
for i = 0, n_elements(ch1_e12b50) - 1 do print, name(ch1_e12b50(i)), corr_cernoxK(ch1_e12b50(i)), new_mean(ch1_e12b50(i)), bias(ch1_e12b50(i)), vgg1(ch1_e12b50(i)), vreset(ch1_e12b50(i)), vdduc(ch1_e12b50(i))
;mean noise

x=cernoxK(ch1_e12b50)
y = new_mean(ch1_e12b50)
plot, x[sort(x)], y[sort(x)] , psym = 2,xtitle ='Cernox Temp (K)', $
      ytitle = 'Mean Noise Level (MJy/sr)', title = 'Bias = 500', xrange=[25,40], yrange=[0.002,.02],$
...
;noisy pix
x = cernoxK(ch1_e100b50)
y = noisypix(ch1_e100b50)
plot, x[sort(x)], y[sort(x)], psym = 2,/ylog, xtitle ='Cernox Temp (K)', $
      ytitle = 'Fraction of Noisy Pixels', title = 'Bias = 500', xrange=[25,40], yrange=[0.0001,.1],$
      thick=3, xthick=3, ythick=3, charthick=3
oplot, x[sort(x)], y[sort(x)]
...
oplot, x[sort(x)], y[sort(x)],  psym = 2, color = bluecolor, thick=3
oplot, x[sort(x)], y[sort(x)],  color = bluecolor
oplot, [3,800], [12./(256.*256.), 12./(256.*256.)],thick=3
xyouts, 35, 12./(256.*256.), 'CR in 12s', charthick=3
oplot, [3,800], [100./(256.*256.), 100./(256.*256.)],thick=3
xyouts, 35, 100./(256.*256.), 'CR in 100s', charthick=3
oplot, [3,800], [200./(256.*256.), 200./(256.*256.)],thick=3
xyouts, 35, 200./(256.*256.), 'CR in 200s', charthick=3
...
ch2_e12t30 = where(channel eq 2 and cernoxK eq 31 and exptime eq 12 and new_mean ne 0 )
ch1_e100t30 = where(channel eq 1 and cernoxK eq 29.4 and exptime eq 100 and new_mean ne 0  )
ch2_e100t30 = where(channel eq 2 and cernoxK eq 31 and exptime eq 100 and new_mean ne 0 )

print, 'est bias', bias(ch1_e100t30)

x=bias(ch1_e12t30)
y = new_mean(ch1_e12t30)
y = y[sort(x)]
x= x[sort(x)]
plot,  x, y, psym = 2, xrange=[400,600], xtitle ='Bias',thick=3, charthick=3, xthick=3, ythick=3,$
      ytitle = 'Mean Noise Level (MJy/sr)', title = ' 100s', yrange=[0.002,.02]
oplot, x, y

;xyouts, 500 ,.04295, 'C', color = bluecolor, charthick=3
;xyouts, 500, .031072, 'C', charthick=3
oplot, findgen(600), fltarr(600)+0.031072, linestyle = 2, thick = 3
oplot, findgen(600), fltarr(600)+0.04295, linestyle = 2, thick = 3, color = bluecolor

;--------------------------------------
;--------------------------------------
;ch1 t32
ch1_e12t31 = where(channel eq 1 and  cernoxK eq 32 and exptime eq 12 and new_mean ne 0 and bias gt 400 and bias lt 600 and vgg1 gt -4)
ch2_e12t31 = where(channel eq 2 and  cernoxK eq 32.1 and exptime eq 12 and new_mean ne 0 )
ch1_e100t31 = where(channel eq 1 and cernoxK eq 32 and exptime eq 100 and new_mean ne 0 and vgg1 gt -4 )
ch2_e100t31 = where(channel eq 2 and cernoxK eq 32.1 and exptime eq 100 and new_mean ne 0 )

for i = 0, n_elements(ch1_e12t31) - 1 do print, bias(ch1_e12t31(i)), vgg1(ch1_e12t31(i)), new_mean(ch1_e12t31(i))
x=bias(ch1_e12t31)
y=new_mean(ch1_e12t31)
...
y = y[sort(x)]
x= x[sort(x)]
oplot,  x, y, psym = 4, color=bluecolor, thick=3;, xrange=[300,800], xtitle ='Bias',thick=3, charthick=3, xthick=3, ythick=3,$
;      ytitle = 'Mean Noise Level (MJy/sr)', title = 'Ch2 12s', yrange=[0.04,.06]
oplot, x, y, color=bluecolor

;ch2 t32
x=bias(ch2_e12t31)
y=new_mean(ch2_e12t31)
oplot,  x[sort(x)], y[sort(x)], psym = 6, color = bluecolor, thick=3; redcolor
oplot,  x[sort(x)], y[sort(x)], color = bluecolor; redcolor

legend, ['28.7K','29.4K ', '31K', '32K '], psym = [7,2,4,6],/right,/top, thick=3, charthick=3
;legend, ['29.4K ch1, 31K ch2', '32K ch1&2'], psym = [2,6],/right,/top, thick=3, charthick=3
;legend, ['Ch1', 'Ch2'], linestyle = [0,0], color = [blackcolor, bluecolor],/left,/top, thick=3, charthick=3
;-----------------------------------------
;add in the new recal values
ch1_e12t287 = where(channel eq 1 and  cernoxK eq 28.7 and exptime eq 12 and new_mean ne 0 )
ch2_e12t287 = where(channel eq 2 and  cernoxK eq 28.7 and exptime eq 12 and new_mean ne 0 )
ch1_e100t287 = where(channel eq 1 and  cernoxK eq 28.7 and exptime eq 100 and new_mean ne 0 )
ch2_e100t287 = where(channel eq 2 and  cernoxK eq 28.7 and exptime eq 100 and new_mean ne 0 )
x=bias(ch2_e12t287)
y=new_mean(ch2_e12t287)
oplot,  x[sort(x)], y[sort(x)], psym = 7, color =bluecolor, thick=3; redcolor
x=bias(ch1_e12t287)
y=new_mean(ch1_e12t287)
oplot,  x[sort(x)], y[sort(x)], psym = 7, thick=3; redcolor
...
plot,x[sort(x)], y[sort(x)], psym = 2, xtitle ='Bias', thick=3, charthick=3, xthick=3, ythick=3,$
      ytitle = 'Fraction of Noisy Pixels', xrange=[400,600], yrange=[0.0,.008]
oplot,x[sort(x)], y[sort(x)]

x=bias(ch1_e100t31)
y=noisypix(ch1_e100t31)
...
oplot,x[sort(x)], y[sort(x)], psym = 4, color=bluecolor, thick=3;,/ylog, xtitle ='Bias', thick=3, charthick=3, xthick=3, ythick=3,$
 ;     ytitle = 'Fraction of Noisy Pixels', title = 'Ch2 ', xrange=[300,800], yrange=[0.0001,1]
oplot,x[sort(x)], y[sort(x)], color=bluecolor

x=bias(ch2_e100t31)
y=noisypix(ch2_e100t31)
oplot, x[sort(x)], y[sort(x)] , psym = 6, color = bluecolor, thick=3
oplot, x[sort(x)], y[sort(x)] ,  color = bluecolor

;--------
;again add in the recal values
x=bias(ch2_e100t287)
y=noisypix(ch2_e100t287)
oplot,  x[sort(x)], y[sort(x)], psym = 7, color =bluecolor, thick=3; redcolor
x=bias(ch1_e100t287)
y=noisypix(ch1_e100t287)
oplot,  x[sort(x)], y[sort(x)], psym = 7 ,thick=3; redcolor




oplot, [300,800], [12./(256.*256.), 12./(256.*256.)],thick=3
xyouts, 470, 12./(256.*256.), 'CR in 12s', charthick=3
oplot, [300,800], [100./(256.*256.), 100./(256.*256.)],thick=3
xyouts, 470, 100./(256.*256.), 'CR in 100s', charthick=3
oplot, [300,800], [200./(256.*256.), 200./(256.*256.)],thick=3
xyouts, 470, 200./(256.*256.), 'CR in 200s', charthick=3
;legend, ['32K', '29.4,31K'], psym = [6,2],/left,/top, thick=3, charthick=3
legend, ['28.7K', '29.4K ', '31K', '32K '], psym = [7,2,4,6],/left,/top, thick=3, charthick=3
...
;legend, ['30K', '29K'], linestyle = [0,0], color = [blackcolor, redcolor],/right,/top1
