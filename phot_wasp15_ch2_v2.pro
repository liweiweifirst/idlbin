...
dirname =  '/Users/jkrick/irac_warm/pcrs_planets/wasp15/'
planethash = hash()
  ...
  for a = 0, 1 do begin; n_elements(aorname) - 1 do begin
     ...
     command  = strcompress( "find ch2/bcd -name '*_bcd.fits' > "+dirname+'bcdlist.txt')
     spawn, command
     command2 =  strcompress("find  ch2/bcd -name '*bunc.fits' > "+dirname + 'bunclist.txt')
     spawn, command2
...
;--------------------------------
      ;correct for pixel phase effect based on pmaps from Jim
       ; file_suffix = ['500x500_0043_120409.fits','0p1s_x4_500x500_0043_120531.fits']
        corrflux = iracpc_pmap_corr(abcdflux,x_center,y_center,ch,/threshold_occ, threshold_val = 20)
 ...
 ;    p3 = plot(timearr- timearr(0), corrfluxarr, '1.')
       ...
       values=list(ra_ref,  dec_ref, xarr, yarr, fluxarr, fluxerrarr, corrfluxarr, corrfluxerrarr, sclk_0, timearr, aorname(a), bmjd, utcs, back, backerrarr)
  ...
  print, 'testing (planethash[aorname(1),xcen])(0)',(planethash[aorname(1),'xcen'])(0)
  
  save, planethash, filename=strcompress(dirname + 'wasp15_phot_ch2.sav')
