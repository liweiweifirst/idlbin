...
!P.multi = [0,1,2]
outputfile = strcompress(dir_name + '/iwic210_12.ps',/remove_all) 
...
;readcol, dir_name + '/aor_list_12', aorname, format="A",/silent
aorname = 'r25131776'
...
;         if aor lt 3 then begin
         if ch eq 0 then begin
            rawlist = strcompress(dir_name + '/ch1/raw/rawlist_ch1_' + exptime(exp)+'s.txt',/remove_all)
            bcdlist =  strcompress(dir_name + '/ch1/bcd/bcdlist_ch1_' + exptime(exp)+'s.txt',/remove_all)
         endif else begin
            rawlist = strcompress(dir_name + '/ch2/raw/rawlist_ch2_' + exptime(exp)+'s.txt',/remove_all)
            bcdlist =  strcompress(dir_name + '/ch2/bcd/bcdlist_ch2_' + exptime(exp)+'s.txt',/remove_all)
         endelse
...
;don't need to use all 50, plus then with dithering I wont get
;any good pixels

rawimage = rawimage[0:12]
bcdimage = bcdimage[0:12]

         print, 'rawimage', rawimage(0)
...
;apply the mask to the data
         for name = 0,  n_elements(rawimage) -1 do begin
            ...
            fits_read, strcompress(dir_name+'/ch'+string(ch+1) + '/raw/'+rawimage(name),/remove_all), rawdata_int, rawheader 
            fits_read, strcompress(dir_name+'/ch'+string(ch+1) + '/bcd/'+bcdimage(name),/remove_all), bcddata, bcdheader      
...
;now plot the histogram of stddevs of all unmasked pixels
         ...
         xcold = xjunk
         ycold = yjunk
         save, xcold, filename = strcompress('/Users/jkrick/iwic/r25131776/xcold'+string(ichan) +'.sav',/remove_all)
         save, ycold, filename = strcompress('/Users/jkrick/iwic/r25131776/ycold'+string(ichan) +'.sav',/remove_all)
...
;         oplot, findgen(10000)/ 100., (result(2))/sqrt(2.*!Pi) * exp(-0.5*((findgen(10000)/100. - (result(0)))/(result(1)))^2.), $
         ...
         xcoldfit = xarr
         ycoldfit = (result(2))/sqrt(2.*!Pi) * exp(-0.5*((xarr - (result(0)))/(result(1)))^2.)
         save, xcoldfit, filename = strcompress('/Users/jkrick/iwic/r25131776/xcoldfit'+string(ichan) +'.sav',/remove_all)
         save, ycoldfit, filename = strcompress('/Users/jkrick/iwic/r25131776/ycoldfit'+string(ichan) +'.sav',/remove_all)
