FUNCTION logbase10,input,badval_in=badval_in,outbadval=outbadval
;+NAME/ONE LINE DESCRIPTION:
;    LOGBASE10 takes the log of a map (properly handling bad pixels).
;
;DESCRIPTION:
;    LOGBASE10 receives a 2-D array as input and returns a 2-D array as
...
;    is the log (to the base 10) of the value of the corresponding
;    pixel in the input array.
;
;CALLING SEQUENCE:
;    result = logbase10( input, badval_in=[...], outbadval=[...] )
;
...
;
;EXAMPLE:
;    result = logbase10(input,badval_in=0.,outbad=outbad)
;#
;COMMON BLOCKS:  none.
;
;LIBRARY CALLS:  none.
...
;    the input image which is greater than 0.  Create result such that
;    all non-good pixels receive a bad value, and all good pixels receive
;    the log (to the base 10) of the corresponding value from the input
;    image.  If any good pixel has received, as a result of the operation,
...
;  ---------------------------------------------------------------------
  outbadval = badval_in
  IF(NOT graphs) THEN BEGIN
    good = WHERE((input NE badval_in) AND (input GT 0.))
    result = input * 0. + outbadval
    IF(good(0) NE -1) THEN result(good) = ALOG10(input(good)) $
                      ELSE RETURN,result
;
...
;  -----------------------------------
    good = WHERE((input(*,1) NE badval_in) AND (input(*,1) GT 0.))
    bad  = WHERE((input(*,1) EQ badval_in) OR  (input(*,1) LE 0.))
    yout = FLTARR(dim1) + outbadval
    IF(good(0) NE -1) THEN yout(good) = ALOG10(input(good,1)) $
