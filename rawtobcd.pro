pro rawtobcd
  ...
  dirname = '/Users/jkrick/irac_warm/barrel_shift/pipeline_test/r45416960/'
  cd,dirname
  command = 'ls ch1/bcd/*_bcd.fits > ch1_bcdlist.txt'
  spawn, command
  command = 'ls ch2/bcd/*_bcd.fits > ch2_bcdlist.txt'
  spawn, command
  command = 'ls ch1/raw/*_dce.fits > ch1_rawlist.txt'
  spawn, command
  command = 'ls ch2/raw/*_dce.fits > ch2_rawlist.txt'
  spawn, command
...
;         print, 'zeros', n_elements(sigmaarr), n_elements(goodsig)
     ...
     a1 = plot(xhist, yhist, '2r', ytitle = 'number', xtitle = 'mean', name = 'raw', title = 'ch2 2s', xrange = [-1,1])
