...
;
  ...
  restore, '/Users/jkrick/irac_warm/initial_drift/centroids.sav'
  ...
  for a = startaor, stopaor, 2 do begin; only the long ones
     ...
     pnx = plot(timearr_n, xarr_n , '1', xtitle = 'Time (Hrs)', title = 'estimated 60min pre AOR',$
               ytitle = 'Normalized X position', _extra = extra, color = colorarr[a], overplot = pnx)
     pny = plot(timearr_n, yarr_n, '1', xtitle = 'Time (Hrs)', title = 'estimated 60min pre AOR',$
               ytitle = 'Normalized Y position', _extra = extra, color = colorarr[a], overplot = pny)
     ...
     first = where((timearr - timearr(0)) / 60. lt 5 and (timearr - timearr(0)) / 60. ge 0,nfirst)
     last = where((timearr - timearr(0)) / 60. lt 240 and (timearr - timearr(0)) / 60. ge 235, nlast)
     if nlast gt 0 then begin  ;;otherwise less than 4 hours in duration
        x_mean_first = mean(xarr(first))
        x_mean_last = mean(xarr(last))
        y_mean_first = mean(yarr(first))
        y_mean_last = mean(yarr(last))
